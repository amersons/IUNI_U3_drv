...
//Gionee <zhuangxiaojian> <2014-08-27> modify for CR01370090 begin
#if ORIGINAL_VERSION
    if (player != 0) {
        player->seekTo(0);
        player->start();
	...
	if (player != 0) {
		...
		AudioSystem::getStreamVolumeIndex(AUDIO_STREAM_NOTIFICATION, 
			&index, AUDIO_DEVICE_OUT_SPEAKER);
		if (index != 0) {
			player->seekTo(0);
			player->start();
		}
...
#endif
//Gionee <zhuangxiaojian> <2014-08-27> modify for CR01370090 end
}
