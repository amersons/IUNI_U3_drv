...
status_t CameraClient::initialize(camera_module_t *module) {
    ...
    enableMsgType(CAMERA_MSG_ERROR | CAMERA_MSG_ZOOM | CAMERA_MSG_FOCUS 
    				| CAMERA_MSG_PREVIEW_METADATA | CAMERA_MSG_FOCUS_MOVE 
					| CAMERA_MSG_GESTURE | CAMERA_MSG_NOTIFY_CONTINUOUS_SHOT
                  	|CAMERA_MSG_SCENE_DETECTION | CAMERA_MSG_GESTURE_DETECTION
...
status_t CameraClient::startRecordingMode() {
    ...
    //Gionee <jiangbo><2013-11-22> modify for CR00956147 begin 
	#ifdef ORIGINAL_VERSION
	...
	#endif
    //Gionee <jiangbo><2013-11-22> modify for CR00956147 end 
...
// Gionee <wutangzhi> <2013-10-28> modify for CR00906596 begin
	...
	mBurstCnt = p.getInt("num-snaps-per-shutter");
    if(mBurstCnt <= 0) mBurstCnt = 1;

	const char* capMode = p.get("cap-mode");
	if (capMode != NULL && strcmp(capMode, "burstshot") == 0) {
...
status_t CameraClient::setParameters(const String8& params) {
    ...
    const char* sound_state = p.get("camera-shutter-sound"); 
    LOG1("sound_state(%s)",sound_state);
	if(sound_state != NULL){
...
void CameraClient::notifyCallback(int32_t msgType, int32_t ext1,
    ...
    switch (msgType) {
		...
		case CAMERA_MSG_NOTIFY_CONTINUOUS_SHOT:
			client->handleContinouseShot();
		#endif
		// Gionee <wutangzhi> <2013-09-13> modify for CR00911795 end
        default:
...
// Gionee <zhuangxiaojian> <2014-06-24> modify for CR01296547 begin
#ifdef ORIGINAL_VERSION
    if (mPlayShutterSound) {
        mCameraService->playSound(CameraService::SOUND_SHUTTER);
    }
#else
	if (mPlayShutterSound && !mLongshotEnabled) {
		mCameraService->playSound(CameraService::SOUND_SHUTTER);
	}
#endif
// Gionee <zhuangxiaojian> <2014-06-24> modify for CR01296547 end
...
// Gionee <wutangzhi> <2013-09-13> modify for CR00911795 begin
void CameraClient::handleContinouseShot() {
    disableMsgType(CAMERA_MSG_POSTVIEW_FRAME);
	disableMsgType(CAMERA_MSG_RAW_IMAGE);
	disableMsgType(CAMERA_MSG_COMPRESSED_IMAGE);
	disableMsgType(CAMERA_MSG_SHUTTER);
