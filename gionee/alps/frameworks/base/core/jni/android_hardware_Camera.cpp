                                ...
                                camera_frame_metadata_t *metadata)
{
	...
	#else
	int32_t dataMsgType = msgType & ~(CAMERA_MSG_PREVIEW_METADATA | CAMERA_MSG_AGEGENDER_DETECTION);
	#endif
	//Gionee <zhuangxiaojian> <2014-10-31> modify for CR01404903 begin
	...
	if (metadata) {
		...
		if (msgType & CAMERA_MSG_AGEGENDER_DETECTION) {
			postMetadata(env, CAMERA_MSG_AGEGENDER_DETECTION, metadata);
		}
	}
	#endif
	//Gionee <zhuangxiaojian> <2014-10-31> modify for CR01404903 end
...
void JNICameraContext::postMetadata(JNIEnv *env, int32_t msgType, camera_frame_metadata_t *metadata)
{
    ...
    for (int i = 0; i < metadata->number_of_faces; i++) {
        ...
        if (mIsExtendedFace) {
			...
			#ifdef ORIGINAL_VERSION
			#else
			env->SetIntField(face, fields.face_age, metadata->faces[i].age);
			env->SetIntField(face, fields.face_gender, metadata->faces[i].gender);
			#endif
			//Gionee <zhuangxiaojian> <2014-10-31> modify for CR01404903 end
        }
...
static void android_hardware_Camera_slowdownContinuousShot(JNIEnv *env, jobject thiz)
{
    ALOGV("slowdown ContinuousShot");
    sp<Camera> camera = get_native_camera(env, thiz, NULL);
    if (camera == 0) return;

    if (camera->sendCommand(CAMERA_CMD_SLOW_CONTINUOUS_SHOT, 0, 0) != NO_ERROR) {
        jniThrowException(env, "java/lang/RuntimeException", "slowdown ContinuousShot failed");
    }
}
...
//Gionee <zhuangxiaojian> <2013-08-28> modify for CR00882216 begin
  ...
  { "slowdownContinuousShot",
  	"()V",
  	(void*)android_hardware_Camera_slowdownContinuousShot},
#endif
//Gionee <zhuangxiaojian> <2013-08-28> modify for CR00882216 end
};
...
int register_android_hardware_Camera(JNIEnv *env)
{
    ...
    field extendedfacefields_to_find[] = {
        ...
        #ifdef ORIGINAL_VERSION
		#else
        { "org/codeaurora/camera/ExtendedFace", "age", "I", &fields.face_age },
        { "org/codeaurora/camera/ExtendedFace", "gender", "I", &fields.face_gender },
        #endif
        //Gionee <zhuangxiaojian> <2014-10-31> modify for CR01404903 end
    };
