...
def OTA_VerifyEnd(info, api_version, target_zip, source_zip=None):
  ...
  global binImages
  global fwImages
  global ext4_fwImages
  bootImages, binImages, fwImages, ext4_fwImages = SplitFwTypes(update_list)
  ...
  if largest_source_size != 0:
    ...
    for f in ext4_fwImages:
      dest, destBak, tf, sf = ext4_fwImages[f]
      # Not incremental
      if sf is None:
        continue
      info.script.AppendExtra('unmount("/firmware");')
      info.script.AppendExtra('mount("ext4", "EMMC", "%s", "/firmware");' %
                                    (dest))
      dest = dest + "/" + f
      info.script.PatchCheck(dest, tf.sha1, sf.sha1)
...
def InstallBootImages(script, files):
  ...
  #comment by wangyongliang for aboot update
...
def Installext4FwImages(script, files):
  last_mounted = ""
  ...
  for f in files:
    ...
    fn = f.split("/")[-1]
    script.AppendExtra('unmount("/firmware");')
    script.AppendExtra('mount("ext4", "EMMC", "%s", "/firmware");' %
                            (dest))
    script.AppendExtra('package_extract_file("%s", "/firmware");' % (f))
...
def OTA_InstallEnd(info):
  ...
  if ext4_fwImages != {}:
    Installext4FwImages(info.script, ext4_fwImages)
  return
