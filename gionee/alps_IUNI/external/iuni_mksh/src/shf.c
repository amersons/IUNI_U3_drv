...
int iuni_write(int fd, char *s, int count)
{
     //if ((fd==2) || (fd==3))	
#if 1
        int i=0;
...
shf_emptybuf(struct shf *shf, int flags)
{
	...
	if (shf->flags & SHF_STRING) {
		...
		if (shf->flags & SHF_WRITING) {
			...
			while (ntowrite > 0) {
				n = iuni_write(shf->fd, buf, ntowrite);
				if (n < 0) {
					if (errno == EINTR &&
...
shf_putchar(int c, struct shf *shf)
{
	...
	if (shf->flags & SHF_UNBUF) {
		...
		while ((n = iuni_write(shf->fd, &cc, 1)) != 1)
			if (n < 0) {
				if (errno == EINTR &&
				    !(shf->flags & SHF_INTERRUPT))
					continue;
