...
public class LightsService {
    ...
    //Gionee wanglei, 2013-10-21, add for torch led, start
    public static final int LIGHT_ID_TORCH = 8;
    //Gionee wanglei, 2013-10-21, add for torch led, end
    ...
    public final class Light {
        ...
        //Gionee <GN_Led> <zhangxiaowei> <2013.6.19> modified for CR00827874 start
        public void receiverSetColor(int color) {
            synchronized (this) {
			Log.d(TAG, " lightservices receiverSetColor_1 = " + Integer.toHexString(color));
			 receiverSetLightLocked(color, LIGHT_FLASH_NONE, 0, 0, 0);
            }
        }
        public void receiversetFlashing(int color, int mode, int onMS, int offMS) {
            synchronized (this) {
            	receiverSetLightLocked(color, mode, onMS, offMS, BRIGHTNESS_MODE_USER);
            }
        }
        private void receiverSetLightLocked(int color, int mode, int onMS, int offMS, int brightnessMode) {
        	Log.d(TAG, "receiverSetLight_1 #" + mId + ": color_1=#"
                        + Integer.toHexString(color));
          setLight_native(mNativePointer, mId, color, mode, onMS, offMS, brightnessMode);
        }
       //Gionee <GN_Led> <zhangxiaowei> <2013.6.19> modified for CR00827874 end
        public void setColor(int color) {
            synchronized (this) {
        		Log.d(TAG, " lightservices SetColor = " + Integer.toHexString(color));
        ...
        private void setLightLocked(int color, int mode, int onMS, int offMS, int brightnessMode) {
            if (color != mColor || mode != mMode || onMS != mOnMS || offMS != mOffMS) {
                if (DEBUG) Slog.v(TAG, "........setLight #" + mId + ": color=#"
                        + Integer.toHexString(color) + "onMS:" + onMS + "offMS" + offMS );
                mColor = color;
                mMode = mode;
                mOnMS = onMS;
                mOffMS = offMS;
