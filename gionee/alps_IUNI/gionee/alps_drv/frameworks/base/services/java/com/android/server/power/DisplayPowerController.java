...
final class DisplayPowerController {
    ...
    //Gionee licz modify for auto brightness begin
    private static final long BRIGHTENING_LIGHT_DEBOUNCE = 500; //4000
    private static final long DARKENING_LIGHT_DEBOUNCE = 1000; //8000
    //Gionee licz modify for auto brightness end
    ...
    public DisplayPowerController(Looper looper, Context context, Notifier notifier,
    	...
    	// qy add 2014 07 03 for auto-brightness begin
    	auroraSetContext(context);
		auroraInitBrightnessObserver(); 
    	// qy add 2014 07 03 for auto-brightness end
...
//aurora add zhouxiaobing 20141031 start
public DisplayPowerController(Looper looper, Context context, Notifier notifier,
            LightsService lights, TwilightService twilight, SensorManager sensorManager,
            DisplayManagerService displayManager,
            SuspendBlocker displaySuspendBlocker, DisplayBlanker displayBlanker,
            Callbacks callbacks, Handler callbackHandler, PowerManagerService p)
   {
           
           this(looper,context,notifier,lights,twilight,sensorManager,displayManager,displaySuspendBlocker,displayBlanker,callbacks,callbackHandler);
           pws=p;
   }
//aurora add zhouxiaobing 20141031 end
    ...
    public boolean requestPowerState(DisplayPowerRequest request,
        ...
        synchronized (mLock) {
            ...
            if (mPendingRequestLocked == null) {
                ...
                changed = true;
            } else if (!mPendingRequestLocked.equals(request)||is_need_update) {
...
//aurora add zhouxiaobing 20141031 start
    boolean is_power_proximity=false;
    boolean is_need_update=false;
    DisplayPowerRequest request_pws;
    PowerManagerService pws;
    public boolean setPowerProximity()
    {
        handler.removeMessages(0);
        if(mProximity == PROXIMITY_POSITIVE)
        {
          is_power_proximity=true;
          is_need_update=true;
          return true;
        }
        return false;      
    ...
    public boolean isCanChangeState()
    {
     ...
     return !(mProximity == PROXIMITY_POSITIVE&&!is_power_proximity);
   }
...
//aurora add zhouxiaobing 20141031 start
        if(!mPowerRequest.useProximitySensor&&mProximity == PROXIMITY_POSITIVE)
        {
//           is_power_proximity=true;
        }    
        ...
        {
         if(mPowerRequest.useProximitySensor||is_power_proximity)
          {
          is_power_proximity=false;
          if(mPowerRequest.screenState != DisplayPowerRequest.SCREEN_STATE_BRIGHT)
          {   
            if(pws.isSleep())
              {
                pws.wakeUp2(SystemClock.uptimeMillis());
                
              }
            else
              { 
               mPowerRequest.screenState=DisplayPowerRequest.SCREEN_STATE_BRIGHT;
               request_pws.screenState=DisplayPowerRequest.SCREEN_STATE_BRIGHT;
               PowerManagerService.SetPowerState(true,true);
              }
          }
          }  
        }
        if(is_power_proximity)
        {
           handler.sendMessageDelayed(handler.obtainMessage(0), 3000);
        }   
        Log.v("hahajlu","updatepowerstate mPowerRequest.useProximitySensor="+mPowerRequest.useProximitySensor+" mProximity="+mProximity+" is_power_proximity="+is_power_proximity+" mPowerRequest.screenState="+mPowerRequest.screenState+" mScreenOffBecauseOfProximity="+mScreenOffBecauseOfProximity);
//aurora add zhouxiaobing 20141031 end
        ...
        if (mProximitySensor != null) {
            ...
            if (mPowerRequest.useProximitySensor
                ...
                //aurora add zhouxiaobing 20141031 start
                    mPowerRequest.screenState = DisplayPowerRequest.SCREEN_STATE_OFF;
                    request_pws.screenState = DisplayPowerRequest.SCREEN_STATE_OFF;
                    PowerManagerService.SetPowerState(false,false);
               //aurora add zhouxiaobing 20141031 end
                    setScreenOn(false);
                }
                    ...
                    && mProximity == PROXIMITY_POSITIVE
                    && mPowerRequest.screenState != DisplayPowerRequest.SCREEN_STATE_OFF) {
                Slog.d(TAG, "updatePowerState 2");
                setProximitySensorEnabled(true);
            } else {
                Slog.d(TAG, "updatePowerState 3");
        ...
        if (!mScreenOffBecauseOfProximity||is_power_proximity) {////aurora add zhouxiaobing 20141031
            if (wantScreenOn(mPowerRequest.screenState)) {
                // Want screen on.
    ...
    private void setProximitySensorEnabled(boolean enable) {
        if (enable) {
            if (!mProximitySensorEnabled) {
                ...
                Slog.d(TAG, "mProximitySensorEnabled true");
            ...
            if (mProximitySensorEnabled) {
                ...
                Slog.d(TAG, "mProximitySensorEnabled false");
    ...
    private void debounceProximitySensor() {
        if (mProximitySensorEnabled
            ...
            if (mPendingProximityDebounceTime <= now) {
                ...
                Slog.d(TAG, "debounceProximitySensor mProximity = " + mProximity);
                updatePowerState();
                clearPendingProximityDebounceTime(); // release wake lock (must be last)
            } else {
 ...
 // qy del 2014 07 03 for auto-brightness begin
    private void setLightSensorEnabled(boolean enable, boolean updateAutoBrightness) {
    	auroraSetLightSensorEnabled(enable, updateAutoBrightness);
    }
 ...
 // qy add 2014 07 03 for auto-brightness end 
    ...
    private void updateAutoBrightness(boolean sendUpdate) {
        ...
        int newScreenAutoBrightness = clampScreenBrightness(
         ...
         // qy modify 2014 07 03 for auto-brightness begin    
        newScreenAutoBrightness = auroraGetBrightnessValue();
        // qy modify 2014 07 03 for auto-brightness end       
	...
	// qy add 2014 07 03 for auto-brightness begin
    public Context auroraContext;
    private void auroraSetContext(Context context){
    	
    	auroraContext = context;
    }
    private Context auroraGetContext(){
    	return auroraContext;
    }
	
	private boolean auroraGetContentObserverState(){
		if(mPowerRequest !=null){
			return 	mPowerRequest.useAutoBrightness
	...
	private void auroraSetLightSensorEnabled(boolean enable, boolean updateAutoBrightness) {
    	if (enable) {
            if (!mLightSensorEnabled) {
                updateAutoBrightness = true;
             ...
             // qy del 2014 07 03 for auto-brightness end
            }
        } else {
            if (mLightSensorEnabled) {
...
//                mSensorManager.unregisterListener(mLightSensorListener);
    ...
    public final class AuroraBrightnessObserver extends android.database.ContentObserver{
    	...
    	public void onChange(boolean selfChange) {
    		// TODO Auto-generated method stub
			/*
    		int br = Settings.System.getInt(auroraContext.getContentResolver(),
                    Settings.System.SCREEN_BRIGHTNESS, 100);
        	animateScreenBrightness(clampScreenBrightness(br),
                     BRIGHTNESS_RAMP_RATE_FAST);
			*/
				auroraOnChange(selfChange);
        	}

		public void auroraOnChange(boolean selfChange) {
    		// TODO Auto-generated method stub
    		int br = Settings.System.getInt(auroraContext.getContentResolver(),
                    Settings.System.SCREEN_BRIGHTNESS, 100);
        	auroraSetBrightnessValue(br);
        	}
		}
    
    
	public void auroraSetBrightnessValue(int value){
		animateScreenBrightness(clampScreenBrightness(value),
                    /*slow ? BRIGHTNESS_RAMP_RATE_SLOW :*/ BRIGHTNESS_RAMP_RATE_FAST);
	}
    private AuroraBrightnessObserver auroraBrightnessObserver;

	private void auroraInitBrightnessObserver(){
		
		auroraBrightnessObserver = new AuroraBrightnessObserver(new Handler());
	}
    

    private void auroraSetContentObserver(){
		boolean enable = auroraGetContentObserverState();
    	if(enable){
    		auroraContext.getContentResolver().registerContentObserver(
                    Settings.System.getUriFor(Settings.System.SCREEN_BRIGHTNESS), true,
                    auroraBrightnessObserver);
    	}else{
    		auroraContext.getContentResolver().unregisterContentObserver(auroraBrightnessObserver);
    	}
    ...
    private int auroraGetBrightnessValue(){
    	return Settings.System.getInt(auroraContext.getContentResolver(),
                Settings.System.SCREEN_BRIGHTNESS, 100);
    }
