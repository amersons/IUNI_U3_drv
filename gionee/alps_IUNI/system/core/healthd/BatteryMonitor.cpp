...
//wolfu add for usb over voltage
int getUsbHealth(const char* status) {
    int ret;
    struct sysfsStringEnumMap usbHealthMap[] = {
        { "Unknown", BATTERY_HEALTH_UNKNOWN },
        { "Good", BATTERY_HEALTH_GOOD },
        { "Over voltage", BATTERY_HEALTH_OVER_VOLTAGE },
        { NULL, 0 },
    ...
    ret = mapSysfsString(status, usbHealthMap);
    if (ret < 0) {
        KLOG_WARNING(LOG_TAG, "Unknown usb health '%s'\n", status);
        ret = BATTERY_HEALTH_UNKNOWN;
    }
...
bool BatteryMonitor::update(void) {
    ...
    if (readFromFile(mHealthdConfig->usbHealthPath, buf, SIZE) > 0)
        props.batteryHealth = getUsbHealth(buf);

    if(props.batteryHealth != BATTERY_HEALTH_OVER_VOLTAGE)
    {
        ...
        while ((entry = readdir(dir))) {
            ...
            case ANDROID_POWER_SUPPLY_TYPE_USB:
                if (mHealthdConfig->usbHealthPath.isEmpty()) {
                    path.clear();
//                    path.appendFormat("%s/%s/health", "/data", "usb");
            ...
            case ANDROID_POWER_SUPPLY_TYPE_BATTERY:
                ...
                if (mHealthdConfig->batteryHealthPath.isEmpty()) {
                    path.clear();
//                    path.appendFormat("%s/%s/health", "/data", "battery");
                    path.appendFormat("%s/%s/health", POWER_SUPPLY_SYSFS_PATH, name);
                    if (access(path, R_OK) == 0)
