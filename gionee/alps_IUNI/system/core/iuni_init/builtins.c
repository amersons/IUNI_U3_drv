...
void do_charger_mode()
{
    struct service *adbd_svc = service_find_by_name("adbd");
    struct service *rmts_svc = service_find_by_name("rmt_storage");
    char tmp[PROP_VALUE_MAX];
    //const char * tmp2;
    
    if (adbd_svc != NULL){
        service_start(adbd_svc, NULL);
    }
    service_for_each_class("charger", service_start_if_not_disabled);

    if ((property_get("ro.emmc", tmp)) != 0 && !strcmp(tmp, "1") && rmts_svc != NULL){
        service_start(rmts_svc, NULL);
    }
}
#endif
...
//Gionee wudp 2013-07-05 add for support poweroff chg begin
#if defined(CONFIG_GN_Q_BSP_PM_POWEROFF_CHG_SUPPORT)  
    static int one_shot_charger = 0;
    if (strcmp(args[1], "charger") == 0){
        if (one_shot_charger == 0){
            one_shot_charger = 1;
            do_charger_mode();
        }
        return 0;
