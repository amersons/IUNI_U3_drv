...
#include "gn_synaptics_dsx_core.h"
...
#ifdef DRAW_GESTURES
unsigned char flag_coordinate[24] = {0};
int start_x_coordinate = 0;
int start_y_coordinate = 0;
int end_x_coordinate = 0;
int end_y_coordinate = 0;
int minx_x_coordinate = 0;
int minx_y_coordinate = 0;
int miny_x_coordinate = 0;
int miny_y_coordinate = 0;
int maxy_x_coordinate = 0;
int maxy_y_coordinate = 0;
int maxx_x_coordinate = 0;
int maxx_y_coordinate = 0;

static void coordinate_read_function(unsigned char  data[]);
#endif
		...
		const char *buf, size_t size)
{
	int rt;
	unsigned long val;
    unsigned char test_wake_flag = 0;
	unsigned char device_ctrl[3] = {0};
	...
	printk("%s, %d\n", __func__, wake_switch);
       ...
       if (2 == wake_switch) {
           synaptics_rmi4_reg_read(test_rmi4_data, 0x0005, &test_wake_flag, 1);
           printk("tpd: test_wake_flag ====> %d\n", test_wake_flag);
       }

       if (3 == wake_switch) {
           device_ctrl[2] = 0x00;
	    synaptics_rmi4_reg_write(test_rmi4_data,
			0x001c,
			device_ctrl,
			sizeof(device_ctrl));
       }
...
static ssize_t tp_gesture_switch_show(struct device *dev,
		...
		char *buf)
{
	return sprintf(buf, "%d,(%d,%d),(%d,%d),(%d,%d),(%d,%d),(%d,%d),(%d,%d)\n", gesture_switch,start_x_coordinate,start_y_coordinate,end_x_coordinate,end_y_coordinate,miny_x_coordinate,miny_y_coordinate,minx_x_coordinate,minx_y_coordinate,maxy_x_coordinate,maxy_y_coordinate,maxx_x_coordinate,maxx_y_coordinate);
}
...
static ssize_t tp_cross_switch_show(struct device *dev,
		...
		char *buf)
{
	return sprintf(buf, "%d,(%d,%d),(%d,%d)\n", cross_switch,start_x_coordinate,start_y_coordinate,end_x_coordinate,end_y_coordinate);
}
...
static ssize_t tp_character_o_switch_show(struct device *dev,
		...
		char *buf)
{
	return sprintf(buf, "%d,(%d,%d),(%d,%d),(%d,%d),(%d,%d),(%d,%d),(%d,%d)\n", character_o_switch,start_x_coordinate,start_y_coordinate,end_x_coordinate,end_y_coordinate,miny_x_coordinate,miny_y_coordinate,minx_x_coordinate,minx_y_coordinate,maxy_x_coordinate,maxy_y_coordinate,maxx_x_coordinate,maxx_y_coordinate);
}
...
static unsigned char glove_status = 0xff;
...
void if_enable_tp_glove( unsigned char enable)
{
	...
	if(enable) {
   	    glove_ctrl2[0] = (enable | 0x25);
	} else {
	    glove_ctrl2[0] = (enable | 0x05);
	}

    retval = synaptics_rmi4_reg_write(glove_rmi4_data,
   		   0x001e,
   		   glove_ctrl2,
   		   sizeof(glove_ctrl2));
   	  if (retval < 0) {
	...
	glove_ctrl2[0] = 0xff;
    synaptics_rmi4_reg_read(glove_rmi4_data,
			0x001e,
			glove_ctrl2,
			sizeof(glove_ctrl2));


    printk("tpd: glove_ctrl2[0] = 0x%x\n", glove_ctrl2[0]);
	glove_switch = (glove_ctrl2[0] == 0x25)?0x01:0x00;	
    glove_status = glove_switch;
    printk("tpd: glove_switch = %d,enable = %d,glove_status = %d\n",glove_switch,enable,glove_status);
    return ;
#endif
}
...
#endif
//Gionee luliya 2014-5-30 add begin 
#ifdef TP_GLOVE_SUPPORT
...
#endif
//Gionee luliya 2014-5-30 add end
...
//Gionee luliya 2014-5-30 modify begin 
#ifdef CONFIG_FB
static void configure_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval)
		printk("tpd Unable to register fb_notifier: %d\n", retval);
	return;
}
#elif defined CONFIG_HAS_EARLYSUSPEND
static void configure_sleep(struct synaptics_rmi4_data *rmi4_data)
...
#endif
//Gionee luliya 2014-5-30 modify end
...
static void coordinate_read_function(unsigned char  data[])
{
	   start_x_coordinate = data[1]*256 + data[0];
	   start_y_coordinate = data[3]*256 + data[2];
	   end_x_coordinate = data[5]*256 + data[4];
	   end_y_coordinate = data[7]*256 + data[6];
    if (gesture_switch || character_o_switch) {
	   miny_x_coordinate = data[9]*256 + data[8];
	   miny_y_coordinate = data[11]*256 + data[10];
	   minx_x_coordinate = data[13]*256 + data[12];
	   minx_y_coordinate = data[15]*256 + data[14];
       maxy_x_coordinate = data[17]*256 + data[16];
	   maxy_y_coordinate = data[19]*256 + data[18];
	   maxx_x_coordinate = data[21]*256 + data[20];
	   maxx_y_coordinate = data[23]*256 + data[22];
    }
}
#endif
...
static void synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	struct synaptics_rmi4_device_info *rmi;
#ifdef DOUBLE_CLICK_WAKE
       unsigned char  double_wake_flag[4] = {0};
       int cross_flag = 0;
#endif
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: tpd Failed to read interrupt status\n",
				__func__);
		return;
	}
       ...
       if (wake_suspend_compelete) {
           if (wake_switch || gesture_switch || cross_switch || character_o_switch) {
			    ...
			    0x0008,
			    double_wake_flag,
			    sizeof(double_wake_flag));
                printk("double_wake_flag = %d\n", double_wake_flag[0]);
            }

            //double wake
           if (wake_switch) {
                if (double_wake_flag[0] == 0x03) {
           ...
           //gesture U
           if (gesture_switch) {
		   	   printk("luliya tpd gesture switch enter double_wake_flag[0]= %d,double_wake_flag[2] = %d\n",double_wake_flag[0],double_wake_flag[2]);
               if (double_wake_flag[0] == 0x0b && double_wake_flag[2] == 0x63) {
#ifdef DRAW_GESTURES
			   	   retval = synaptics_rmi4_reg_read(rmi4_data,
			                0x0400,
			                flag_coordinate,
			                sizeof(flag_coordinate));
				   printk("luliya tpd flag_coordinate[0] = %d,flag_coordinate[1]= %d,flag_coordinate[2]=%d,flag_coordinate[3]=%d\n",flag_coordinate[0],flag_coordinate[1],flag_coordinate[2],flag_coordinate[3]);
				   coordinate_read_function(flag_coordinate);
#endif
                   GestureU_work(rmi4_data, double_wake_flag[0]);
                   return ;
               }
           }
		   ...
		   if (cross_switch) {
		   	   printk("luliya tpd enter cross_switch \n");
               if (double_wake_flag[0] == 0x07 && ((double_wake_flag[2] > -2) || (double_wake_flag[2] < 2))) {
#ifdef DRAW_GESTURES
			   	   retval = synaptics_rmi4_reg_read(rmi4_data,
			                0x0400,
			                flag_coordinate,
				   ...
				   coordinate_read_function(flag_coordinate);
#endif
			   	   printk("luliya tpd double_wake_flag[1] = %x\n",double_wake_flag[0]);
			   	   if(double_wake_flag[1] & 0x80)
				   	   cross_flag = CROSS_LEFT;
				   else 
				   	   cross_flag = CROSS_RIGHT;
				   printk("luliya tpd cross_flag = %d\n",cross_flag);
           ...
           if (character_o_switch) {
               ...
               if (double_wake_flag[0] ==  0x08) {
				   ...
				   coordinate_read_function(flag_coordinate);
#endif
                   Character_o_work(rmi4_data, double_wake_flag[0]);
                   return ;
               }
           }
		...
		bool enable)
{
	...
	if (enable) {
		...
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: tpd Failed to create irq thread\n",
					__func__);
			return retval;
		}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!f1a) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: TPD Failed to alloc mem for function handle\n",
				__func__);
		return -ENOMEM;
	}
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: TPD Failed to read query registers\n",
				__func__);
		return retval;
	}
	...
	if (!f1a->button_control.txrx_map) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: TPD Failed to alloc mem for tx rx mapping\n",
				__func__);
		return -ENOMEM;
	}
	...
	if (!f1a->button_data_buffer) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: TPD Failed to alloc mem for data buffer\n",
				__func__);
		return -ENOMEM;
	}
	...
	if (!f1a->button_map) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: TPD Failed to alloc mem for button map\n",
				__func__);
		return -ENOMEM;
	}
...
static void synaptics_rmi4_set_configured(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: tpd Failed to set configured\n",
				__func__);
		return;
	}
...
#if defined(CONFIG_GN_DEVICE_TYPE_CHECK) 
       //Read product ID
       retval = synaptics_rmi4_reg_read(rmi4_data, rmi4_data->f01_query_base_addr + 11, product_id, 2);
       if (retval < 0) {
           printk("tpd: read product id failed...\n");
       }else {
           printk("tpd: Product id is %s\n", product_id);
       }

       //Read firmware ID
       retval = synaptics_rmi4_reg_read(rmi4_data, 0x000a, firmware_id, 4);
       if (retval < 0) {
           printk("tpd: read firmware id failed...\n");
       }else {
           printk("tpd: Firmware id is 0x%02x 0x%02x 0x%02x 0x%02x\n", firmware_id[0], firmware_id[1], firmware_id[2], firmware_id[3]);
       }
       snprintf(gn_mydev_info.name, 20, "%s, %02x%02x", product_id, firmware_id[2], firmware_id[3]);
...
flash_prog_mode:
	...
	if (rmi->manufacturer_id != 1) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: TPD Non-Synaptics device found, manufacturer ID = %d\n",
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	const struct synaptics_dsx_board_data *bdata =
				rmi4_data->hw_if->board_data;
    printk("luliya tpd enter synaptics_rmi4_set_input_dev\n");
	rmi4_data->input_dev = input_allocate_device();
	if (rmi4_data->input_dev == NULL) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: TPD Failed to allocate input device\n",
				__func__);
		retval = -ENOMEM;
	...
	if (retval) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: TPD Failed to register input device\n",
				__func__);
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: TPD Failed to configure attention GPIO\n",
				__func__);
		goto err_gpio_irq;
	}
	...
	if (bdata->power_gpio >= 0) {
		...
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: TPD Failed to configure power GPIO\n",
					__func__);
			goto err_gpio_power;
		}
	}
	...
	if (bdata->reset_gpio >= 0) {
		...
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: TPD Failed to configure reset GPIO\n",
					__func__);
			goto err_gpio_reset;
		}
	}
...
//	if ((bdata->pwr_reg_name != NULL) && (*bdata->pwr_reg_name != 0)) {
		...
		if (IS_ERR(rmi4_data->pwr_reg)) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: TPD Failed to get power regulator\n",
					__func__);
...
//	if ((bdata->bus_reg_name != NULL) && (*bdata->bus_reg_name != 0)) {
		rmi4_data->bus_reg = regulator_get(rmi4_data->pdev->dev.parent,
				"vcc_i2c");
        printk("luliya TPD enter synaptics_rmi4_get_reg \n");
		if (IS_ERR(rmi4_data->bus_reg)) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: tpd Failed to get bus pullup regulator\n",
					__func__);
		...
		bool enable)
{
	...
	if (rmi4_data->bus_reg) {
		...
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: TPD Failed to enable bus pullup regulator\n",
					__func__);
			goto exit;
		}
	}
	...
	if (rmi4_data->pwr_reg) {
		...
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: TPD Failed to enable power regulator\n",
					__func__);
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: tpd Failed to issue reset command\n",
				__func__);
		mutex_unlock(&(rmi4_data->rmi4_reset_mutex));
		return retval;
	}
...
static int __devinit synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	if (!hw_if) {
		dev_err(&pdev->dev,
				"%s: TPD No hardware interface found\n",
				__func__);
		return -EINVAL;
	}
	...
	if (!rmi4_data) {
		dev_err(&pdev->dev,
				"%s: TPD Failed to alloc mem for rmi4_data\n",
				__func__);
		return -ENOMEM;
	}
	...
	if (retval < 0) {
		dev_err(&pdev->dev,
				"%s: TPD Failed to get regulators\n",
				__func__);
		goto err_get_reg;
	}
	...
	if (retval < 0) {
		dev_err(&pdev->dev,
				"%s: TPD Failed to enable regulators\n",
				__func__);
		goto err_enable_reg;
	}
	...
	if (hw_if->ui_hw_init) {
		...
		if (retval < 0) {
			dev_err(&pdev->dev,
					"%s: TPD Failed to initialize hardware interface\n",
					__func__);
			goto err_ui_hw_init;
		}
	}
	...
	if (retval < 0) {
		dev_err(&pdev->dev,
				"%s: TPD Failed to set up input device\n",
				__func__);
	...
	if (retval < 0) {
		dev_err(&pdev->dev,
				"%s: TPD Failed to enable attention interrupt\n",
				__func__);
		goto err_enable_irq;
	}
	...
	if (vir_button_map->nbuttons) {
		...
		if (!rmi4_data->board_prop_dir) {
			dev_err(&pdev->dev,
					"%s: TPD Failed to create board_properties directory\n",
					__func__);
			...
			if (retval < 0) {
				dev_err(&pdev->dev,
						"%s: TPD Failed to create virtual key map file\n",
						__func__);
				goto err_virtual_buttons;
			}
		}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if (retval < 0) {
			dev_err(&pdev->dev,
					"%s: TPD Failed to create sysfs attributes\n",
					__func__);
			goto err_sysfs;
		}
	}
...
static void synaptics_rmi4_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: TPD Failed to enter sleep mode\n",
				__func__);
		rmi4_data->sensor_sleep = false;
		return;
	}
...
static void synaptics_rmi4_double_wakeup_enter(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	int count = 0;
	unsigned char button_ctrl;
	unsigned char device_ctrl[3] = {0};

       printk("tpd enter double wakeup...\n");
       do {
            button_ctrl = 0x00;
            retval = synaptics_rmi4_reg_write(rmi4_data,
               0x0005,
	        ...
	        if (retval < 0) {
		       dev_err(&(rmi4_data->input_dev->dev),
			    	"%s tpd: Failed to clear interrupt\n",
			    	__func__);
	        } 
            ...
            synaptics_rmi4_reg_read(rmi4_data,
	    		...
	    		&button_ctrl,
		    	sizeof(button_ctrl));
            printk(" tpd: 0x0005 register = 0x%x\n", button_ctrl);
        }while(button_ctrl != 0x00 && count++ < 10);
       ...
       printk(" tpd 0x001c register = 0x%x\n", device_ctrl[2]);
...
{
	int retval, count = 0;
	unsigned char  int_test = 0;
	unsigned char device_ctrl[3] = {0};

       printk(" tpd: double wakeup exit...\n");
       ...
       do {
	    ...
	    if (retval < 0) {
           ...
           printk(" tpd: 0x001c register = 0x%x\n", device_ctrl[2]);
        }while(device_ctrl[2] != 0x00 && count++ < 10);

       synaptics_rmi4_reg_read(rmi4_data, 0x0005, &int_test, 1);
       printk(" tpd: read int_test = 0x%x\n", int_test);
				...
				unsigned long event, void *data)
{
    ...
    printk("tpd tp_fw_update_compelete = %d\n",tp_fw_update_compelete);
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	int  count = 0;
       ...
       printk("luliya tpd wake_switch = %d,gesture_switch = %d,cross_switch = %d,character_o_switch = %d\n",wake_switch,gesture_switch,cross_switch,character_o_switch);
#ifdef DOUBLE_CLICK_WAKE
       if (wake_switch || gesture_switch || cross_switch || character_o_switch) {
...
#ifdef TP_GLOVE_SUPPORT
	       printk("luliya tpd glove_enable = %d,glove_switch = %d\n",glove_enable,glove_switch);
           if (glove_enable) {
               do {
                   if_enable_tp_glove(0x00);
		   ...
		   printk("luliya tpd rmi4_data->stay_awake = %d\n",rmi4_data->stay_awake);
           return 0;
       }
#endif
