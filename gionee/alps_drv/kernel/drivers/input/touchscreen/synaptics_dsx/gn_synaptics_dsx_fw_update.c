		...
		const unsigned char *fw_image)
{
	...
	printk("luliya tpd header->contains_firmware_id = %d,data->firmware_id[0] = %d,data->firmware_id[1] = %d,data->firmware_id[2] = %d,data->firmware_id[3] = %d",header->contains_firmware_id,data->firmware_id[0],data->firmware_id[1],data->firmware_id[2],data->firmware_id[3]);
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
    ...
    printk("luliya tpd header->contains_firmware_id = %d,fwu->image_name = %s\n",header->contains_firmware_id,fwu->image_name);
	if (header->contains_firmware_id) {
    ...
    printk("luliya tpd image_fw_id = %lu,device_fw_id= %u\n",image_fw_id,device_fw_id);
	if (image_fw_id > device_fw_id) {
		flash_area = UI_FIRMWARE;
		goto exit;
	} else if (image_fw_id < device_fw_id) {
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: tpd Image firmware ID older than device firmware ID\n",
				__func__);
		flash_area = NONE;
		goto exit;
	}
	...
	dev_info(rmi4_data->pdev->dev.parent,
			...
			fwu->config_data[2],
			fwu->config_data[3]);
    printk("luliya tpd image_config_id = %d,device_config_id= %d\n",image_config_id,device_config_id);
		...
		unsigned char command)
{
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s:tpd Failed to write to block number registers\n",
				__func__);
		return retval;
	}
	...
	for (block_num = 0; block_num < block_cnt; block_num++) {
		...
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: tpd Failed to write block data (block %d)\n",
					__func__, block_num);
			return retval;
		}
		...
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: tpd Failed to write command for block %d\n",
					__func__, block_num);
			return retval;
		}
		...
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s:tpd Failed to wait for idle status (block %d)\n",
					__func__, block_num);
			return retval;
		}
...
static int fwu_do_reflash(void)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
    printk("luliya tpd enter fwu_do_reflash\n");
	retval = fwu_write_bootloader_id();
	if (retval < 0)
		return retval;
...
static int fwu_do_lockdown(void)
{
	...
	if (retval < 0)
		return retval;
    printk("luliya tpd enter fwu_do_lockdown\n");
	retval = synaptics_rmi4_reg_read(rmi4_data,
			fwu->f34_fd.query_base_addr + fwu->properties_off,
			&fwu->flash_properties,
			sizeof(fwu->flash_properties));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: tpd Failed to read flash properties\n",
				__func__);
		return retval;
	}
    printk("luliya tpd enter fwu_do_lockdown retval = %d,fwu->flash_properties = %d,UNLOCKED = %d\n",retval,fwu->flash_properties,UNLOCKED);

	if ((fwu->flash_properties & UNLOCKED) == 0) {
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: tpd Device already locked down\n",
				__func__);
		return retval;
	}
...
static int fwu_start_reflash(void)
{
	...
	if (!fwu->rmi4_data->hw_if->board_data->fw_image_name1 
            && !fwu->rmi4_data->hw_if->board_data->fw_image_name2
            && !fwu->rmi4_data->hw_if->board_data->fw_image_name3
            && !fwu->rmi4_data->hw_if->board_data->fw_image_name4
            && !fwu->rmi4_data->hw_if->board_data->fw_image_name5
            && !fwu->rmi4_data->hw_if->board_data->fw_image_name6
            && !fwu->rmi4_data->hw_if->board_data->fw_image_name7
            && !fwu->rmi4_data->hw_if->board_data->fw_image_name8) {
		retval = 0;
		printk("tpd Firmware image name not given, skipping update\n");
		goto exit;
	}
    ...
    retval = fwu->rmi4_data->hw_if->bus_access->read(fwu->rmi4_data, fwu->rmi4_data->f01_query_base_addr + 11, product_id, 10);
	
	if (fwu->ext_data_source) {
		fw_image = fwu->ext_data_source;
	} else {
		if (!strncmp(product_id, "X1", 2)) {
		    printk("tpd firmware: This is OFG Module.\n");
			strncpy(fwu->image_name, fwu->rmi4_data->hw_if->board_data->fw_image_name1, MAX_IMAGE_NAME_LEN);
		} else if (!strncmp(product_id, "s3", 2)) {
            printk("tpd firmware: This is Truly Module.\n");
			strncpy(fwu->image_name, fwu->rmi4_data->hw_if->board_data->fw_image_name2, MAX_IMAGE_NAME_LEN);
        } else if (!strncmp(product_id, "S3", 2)) {
			printk("tpd firmware: This is Truly Module.\n");
			strncpy(fwu->image_name, fwu->rmi4_data->hw_if->board_data->fw_image_name2, MAX_IMAGE_NAME_LEN);
		} else if (!strncmp(product_id, "TB", 2)) {
            printk("tpd firmware: This is Truly White Module.\n");
			strncpy(fwu->image_name, fwu->rmi4_data->hw_if->board_data->fw_image_name3, MAX_IMAGE_NAME_LEN);
        } else if (!strncmp(product_id, "TW", 2)) {
            printk("tpd firmware: This is Truly White Module.\n");
			strncpy(fwu->image_name, fwu->rmi4_data->hw_if->board_data->fw_image_name4, MAX_IMAGE_NAME_LEN);
        } else if (!strncmp(product_id, "OB", 2)) {
            printk("tpd firmware: This is OFG Black Module.\n");
			strncpy(fwu->image_name, fwu->rmi4_data->hw_if->board_data->fw_image_name5, MAX_IMAGE_NAME_LEN);
        } else if (!strncmp(product_id, "OW", 2)) {
            printk("tpd firmware: This is OFG White Module.\n");
			strncpy(fwu->image_name, fwu->rmi4_data->hw_if->board_data->fw_image_name6, MAX_IMAGE_NAME_LEN);
        } else if (!strncmp(product_id, "OG", 2)) {
            printk("tpd firmware: This is OFG Green Module.\n");
			strncpy(fwu->image_name, fwu->rmi4_data->hw_if->board_data->fw_image_name7, MAX_IMAGE_NAME_LEN);
        } else if (!strncmp(product_id, "TG", 2)) {
            printk("tpd firmware: This is Truly Green Module.\n");
			strncpy(fwu->image_name, fwu->rmi4_data->hw_if->board_data->fw_image_name8, MAX_IMAGE_NAME_LEN);
        }
		...
		if (retval != 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: tpd Firmware image %s not available\n",
					__func__, fwu->image_name);
			retval = -EINVAL;
			goto exit;
		}
    ...
    printk("luliya tpd parse_header suscess! fwu->bl_version = %d,header.bootloader_version = %d\n",fwu->bl_version,header.bootloader_version);

	if (fwu->bl_version != header.bootloader_version) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: tpd Bootloader version mismatch\n",
				__func__);
		retval = -EINVAL;
		goto exit;
	}
	...
	if (retval < 0)
		goto exit;
    printk("luliya tpd  fwu_read_f01_device_status retval = %d, f01_device_status.flash_prog = %d\n",retval,f01_device_status.flash_prog);
	if (f01_device_status.flash_prog) {
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: tpd In flash prog mode\n",
				__func__);
		fwu->in_flash_prog_mode = true;
	} else {
		fwu->in_flash_prog_mode = false;
	}
    printk("luliya tpd fwu->do_lockdown = %d,fwu->bl_version = %d\n",fwu->do_lockdown,fwu->bl_version);
	if (fwu->do_lockdown) {
    ...
    printk("luliya tpd header.firmware_size = %d,header.config_size = %d\n",header.firmware_size,header.config_size);
    ...
    printk("luliya tpd header.contains_bootloader = %d\n",header.contains_bootloader);
	if (header.contains_bootloader) {
		if (header.firmware_size)
    ...
    printk("luliya tpd flash_area = %d\n",flash_area);
	switch (flash_area) {
	case UI_FIRMWARE:
