...
//gionee, chuqf, simulate sdcard free size threshold, begin
...
#endif
//gionee, chuqf, simulate sdcard free size threshold, end
...
//gionee, chuqf, simulate sdcard free size threshold, begin
#ifdef LIMIT_SDCARD_SIZE
#define INTERNAL_SDCARD_FREE_SIZE_THRESHOLD           (512*1024*1024)
...
#ifdef OTG_DEVICE_WRITE_SUPPORT
    if (appid == AID_SHELL) {
...
//gionee, chuqf, simulate sdcard free size threshold, begin
	...
	if(!strncmp( fuse->root.name,"/data/media",fuse->root.namelen)){
		...
		if(fuse->free_size <= internal_sdcard_free_size_threshold){
			...
			errno = ENOSPC;
			LOG("[fuse_debug] Oops fuse.free_size = %lld, less than internal sdcard free size threshold = %lld,no space for write!!!!\n",fuse->free_size, internal_sdcard_free_size_threshold);
			return -errno;
		}
	}
#endif
...
//gionee, chuqf, simulate sdcard free size threshold, begin
#ifdef LIMIT_SDCARD_SIZE
	internal_sdcard_free_size_threshold = INTERNAL_SDCARD_FREE_SIZE_THRESHOLD;
#endif
//gionee, chuqf, simulate sdcard free size threshold, end
