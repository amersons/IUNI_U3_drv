...
//Gionee liujiang 2013-11-30 add for invaild data start
		  if( (sam_mag_cal_rpt_ptr->result.m[1] == 0) &&
		  	  (sam_mag_cal_rpt_ptr->result.m[0] == 0) &&
		  	  (sam_mag_cal_rpt_ptr->result.m[2] == 0) )
		  {
		  	  error = true;
		  }
//Gionee liujiang 2013-11-30 add for invaild data end
          break;
...
//Gionee liujiang 2014-08-14 modify for debug_log
  HAL_LOG_ERROR( "%s: handle=%d, enabled=%d", __FUNCTION__, handle, enabled );
...
//Gionee liujiang 2014-08-14 modify for debug_log 
  HAL_LOG_ERROR( "%s: handle %d, sample rate:%d report rate:%d WuFF:%d buffering:%d ignore_prev:%d",
