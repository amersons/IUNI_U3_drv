...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
		...
		if((format == AUDIO_FORMAT_PCM_24_BIT) ||
		   (format == AUDIO_FORMAT_PCM_32_BIT) ||
		   (format == AUDIO_FORMAT_PCM_8_24_BIT)){
			ALOGE("AudioTrack audio_bytes_per_sample:%d", audio_bytes_per_sample(format));
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
		...
		frameCount = sharedBuffer->size()/mChannelCount/audio_bytes_per_sample(format);
#else
...
{
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef AUDIOTRACK_PCM_DUMP
	FILE *fp;
	if ((fp = fopen("/data/atw.cap", "a+")) != NULL) {
		fwrite((uint8_t *)buffer, 1, userSize, fp);
		fclose(fp);
	}else{
		ALOGE("32bits atw.cap OPEN FAILED");
	}
 #endif
 //GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI end
...
#ifdef AUDIOTRACK_PCM_DUMP
		FILE *fp;
		if ((fp = fopen("/data/at.cap", "a+")) != NULL) {
			fwrite((uint8_t *)audioBuffer.i8, 1, audioBuffer.size, fp);
			fclose(fp);
		}else{
			ALOGE("at.cap OPEN FAILED");
		}
#endif
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI end
        releaseBuffer(&audioBuffer);
