...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
    ...
    void        setInBuffer(int32_t *buffer) { mConfig.inputCfg.buffer.s32 = buffer; }
    int32_t     *inBuffer() { return mConfig.inputCfg.buffer.s32; }
    void        setOutBuffer(int32_t *buffer) { mConfig.outputCfg.buffer.s32 = buffer; }
    int32_t     *outBuffer() { return mConfig.outputCfg.buffer.s32; }
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
    void setInBuffer(int32_t *buffer, bool ownsBuffer = false) {
        mInBuffer = buffer;
        mOwnInBuffer = ownsBuffer;
    }
    int32_t *inBuffer() const {
        return mInBuffer;
    }
    void setOutBuffer(int32_t *buffer) {
        mOutBuffer = buffer;
    }
    int32_t *outBuffer() const {
        return mOutBuffer;
    }
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
    int32_t *mInBuffer;         // chain input buffer
    int32_t *mOutBuffer;        // chain output buffer
#else
