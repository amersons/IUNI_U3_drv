...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
    AudioGioneeMixer* mixer = NULL;
    int *mixBuffer = NULL;
    int* MixBufferHAL = NULL;
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
                    mixer = new AudioGioneeMixer(frameCount, sampleRate);
                    mixBuffer = new int[frameCount * FCC_2];
                    MixBufferHAL = new int[frameCount * FCC_2];
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
                    int IsMixerDoPostProc;
                    IsMixerDoPostProc = i==0?0 : 1;
#endif
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI end
                    if (mixer != NULL) {
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
                        name = mixer->getTrackName(AUDIO_CHANNEL_OUT_STEREO, AUDIO_FORMAT_PCM_16_BIT, -555);
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
 #ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
                        ALOGD("fastTrack->mFormat %d", fastTrack->mFormat);
                        mixer->setParameter(
                                    name,
                                    AudioGioneeMixer::TRACK,
                                    AudioGioneeMixer::FORMAT, (void *)fastTrack->mFormat);
                        ALOGD("set resampler");
                        mixer->setParameter(name, AudioGioneeMixer::RESAMPLE,
                                AudioGioneeMixer::SAMPLE_RATE, (void*) fastTrack->mSampleRate);
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
                memset(mixBuffer, 0, frameCount * FCC_2 * sizeof(int));
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
            ...
            if (format == AUDIO_FORMAT_PCM_16_BIT) {
                ...
                int16_t *pDest16;
                int32_t *pSrc32;
                pDest16 = reinterpret_cast<int16_t*>(MixBufferHAL);
                pSrc32 = mixBuffer;
                ...
                framesWritten = outputSink->write(MixBufferHAL, frameCount);
            }
            else {
                ALOGD("fastmixer write");
                framesWritten = outputSink->write(mixBuffer, frameCount);
            }
#else
