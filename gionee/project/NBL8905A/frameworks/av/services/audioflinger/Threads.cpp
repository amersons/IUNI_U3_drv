...
//Gionee Drvier tangwei 20140925 modify for dts start
#if 0
...
#include "postpro_patch.h"
#endif
//Gionee Drvier tangwei 20140925 modify for dts end
...
//Gionee Huangzhuolin 20140804 @ add for CR01338037 begin
#ifdef CONFIG_GN_Q_BSP_AUDIO_SUPPORT
} kUseFastMixer = FastMixer_Never;
#else
} kUseFastMixer = FastMixer_Static;
#endif
//Gionee Huangzhuolin 20140804 @ add for CR01338037 end
...
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
        ,mMixBufferHAL(NULL)
        ,mLowLatencyHalBufferSize(0)
        ,mLowLatencyFrameCount(0)
        ,mScreenStateForChangeHalBuffer(0xffff^AudioFlinger::mScreenState)
#endif
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI end
{
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER	
	if(mFormat == PCM_FORMAT_S32_LE){
		mFrameSize = 8;
		mFormat = AUDIO_FORMAT_PCM_32_BIT;
	}else if(mFormat == PCM_FORMAT_S16_LE){
		mFrameSize = 4;
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin	
#ifdef THREAD_PCM_DUMP
    {
		size_t count = mBytesRemaining >> 3;
        size_t offset = (mCurrentWriteLength - mBytesRemaining) >> 2;
		FILE *fp;
		if ((fp = fopen("/data/w0.cap", "a+")) != NULL) {
			fwrite((uint8_t *)(mMixBuffer + offset), 1, count * 4 * 2, fp);
			fclose(fp);
		}else{
			ALOGE("w0.cap OPEN FAILED");
		}
    }
#endif
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI end
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
		 ALOGVV("mSampleRate=%u, mChannelMask=%#x, mChannelCount=%u, mFormat=%d, mFrameSize=%u, "
...
#ifdef THREAD_PCM_DUMP
			FILE *fp;
			if ((fp = fopen("/data/w1.cap", "a+")) != NULL) {
				fwrite((uint8_t *)(mMixBuffer + offset), 1, count * 4 * 2, fp);
				fclose(fp);
			 }else{
				ALOGE("w1.cap OPEN FAILED");
			 }
#endif
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
    mAudioMixer = new AudioGioneeMixer(mNormalFrameCount, mSampleRate);
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
    ...
    AudioParameter param = AudioParameter(keyLowLatency);
    char *returnString= mOutput->stream->common.get_parameters(&mOutput->stream->common, param.toString());
    AudioParameter returnParam = AudioParameter(String8(returnString));
    int value = 0;

    if (returnParam.getInt(keyLowLatency, value) == NO_ERROR)
    {
        mLowLatencyHalBufferSize = value;
        mLowLatencyFrameCount = mLowLatencyHalBufferSize/8;
    }
    ALOGD("mLowLatencyHalBufferSize %d, mLowLatencyFrameCount %d", mLowLatencyHalBufferSize, mLowLatencyFrameCount);
#endif
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI end
}
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
            if(mLowLatencyHalBufferSize && (mOutput->audioHwDev == mAudioFlinger->mPrimaryHardwareDev)) {
                if(mixBufferSize == mLowLatencyHalBufferSize)
                    sleepTime *= 3 ;
            }
#endif
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI end
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
    //ALOGD("screen %d %d, count %d", mScreenStateForChangeHalBuffer, AudioFlinger::mScreenState, count);
    // enable dynamical change hal buffer size & primary output & active track >= 1
    if(mLowLatencyHalBufferSize && (mOutput->audioHwDev == mAudioFlinger->mPrimaryHardwareDev) && count>=1) {

        size_t buffersize = mLowLatencyHalBufferSize;
        sp<Track> t = mActiveTracks[0].promote();
        Track* const track = t.get();

        if(mScreenStateForChangeHalBuffer != AudioFlinger::mScreenState) {
            if (1==(AudioFlinger::mScreenState&0x1))  {  // screen off
                if(1==count) { // if track number reduce to one, it can change to big hal buffer
                    mScreenStateForChangeHalBuffer = AudioFlinger::mScreenState;
                    // if it not music type or have effect, it will no chance to change to big hal buffer
                    //if(AUDIO_STREAM_MUSIC==track->mStreamType && false==IsEffectsEnable()) //todo...
					        buffersize = mOutput->stream->common.get_buffer_size(&mOutput->stream->common);
						// }
            }
            else {  // screen on
                mScreenStateForChangeHalBuffer = AudioFlinger::mScreenState;
            ...
            if(mixBufferSize != mLowLatencyHalBufferSize){ // big hal buffer  (screen off)
               if (1==count && AUDIO_STREAM_MUSIC==track->mStreamType)  // screen off & only one track (suppose it can't enable effect in screen off)
                    buffersize = mixBufferSize;
                else
                    mScreenStateForChangeHalBuffer = 0xffff^AudioFlinger::mScreenState;
            }
        }

        //ALOGD("buffersize %d %d", buffersize, mixBufferSize);
        if(buffersize != mixBufferSize)
        {
            mixBufferSize = buffersize;
            mNormalFrameCount = mFrameCount = mixBufferSize/mFrameSize;
            cacheParameters_l();
            mAudioMixer->setParameter(NULL, AudioGioneeMixer::BUFFER, NULL, (void *)mNormalFrameCount);
            ALOGD("%s, gScreenState %d, mOutDevice %d, change hal bufferbb %dk, count %d", __FUNCTION__, AudioFlinger::mScreenState, mOutDevice, mixBufferSize, count);
