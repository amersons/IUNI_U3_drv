...
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
    sp<ThreadBase> thread = mThread.promote();
    PlaybackThread *pThread = (PlaybackThread *)thread.get();
    //ALOGD("%s, framesReady() %d, mFrameCount %d %d, flags %d %d", __FUNCTION__, framesReady(),
	//mFrameCount, (pThread->mLowLatencyHalBufferSize/pThread->mFrameSize), mCblk->mFlags, CBLK_FORCEREADY);
    if (pThread->mLowLatencyHalBufferSize) {
        if (framesReady() >= (pThread->mLowLatencyHalBufferSize/pThread->mFrameSize) ||
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
bool AudioFlinger::PlaybackThread::OutputTrack::write(int32_t* data, uint32_t frames)
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
                    pInBuffer->mBuffer = new int32_t[startFrames * channelCount];
                    pInBuffer->frameCount = startFrames;
                    pInBuffer->i32 = pInBuffer->mBuffer;
                    memset(pInBuffer->raw, 0, startFrames * channelCount * sizeof(int32_t));
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
        memcpy(mOutBuffer.raw, pInBuffer->raw, outFrames * channelCount * sizeof(int32_t));
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
        ...
        pInBuffer->i32 += outFrames * channelCount;
        mOutBuffer.frameCount -= outFrames;
        mOutBuffer.i32 += outFrames * channelCount;
#else
        pInBuffer->frameCount -= outFrames;
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
                pInBuffer->mBuffer = new int32_t[inBuffer.frameCount * channelCount];
                pInBuffer->frameCount = inBuffer.frameCount;
                pInBuffer->i32 = pInBuffer->mBuffer;
                memcpy(pInBuffer->raw, inBuffer.raw, inBuffer.frameCount * channelCount * sizeof(int32_t));
#else
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef GIONEE_HIFI_24BITS_AUDIOFLINGER
            pInBuffer->mBuffer = new int32_t[frames * channelCount];
            pInBuffer->frameCount = frames;
            pInBuffer->i32 = pInBuffer->mBuffer;
            memset(pInBuffer->raw, 0, frames * channelCount * sizeof(int32_t));
#else
