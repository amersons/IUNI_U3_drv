...
SRS_Source_CSHP::SRS_Source_CSHP(){
    ...
    pObjBuffer = malloc(SRS_CSHP_GetObjSize());
...
void SRS_Source_CSHP::Create(SRS_Source_Out *pOut){
    ...
    SRS_CSHP_CreateObj(&Obj, pObjBuffer, NULL, NULL);

    if (SampleRate < 19000) SRS_CSHP_InitObj16k(Obj);
    else if (SampleRate < 23000) SRS_CSHP_InitObj22k(Obj);
    else if (SampleRate < 28000) SRS_CSHP_InitObj24k(Obj);
    else if (SampleRate < 38000) SRS_CSHP_InitObj32k(Obj);
    else if (SampleRate < 46000) SRS_CSHP_InitObj44k(Obj);
    else SRS_CSHP_InitObj48k(Obj);

    SRS_CSHP_SetControlDefaults(Obj);
...
void SRS_Source_CSHP::Config(SRS_Source_Out *pOut, SRS_Tech_CSHP *pCFG, bool bBypass){
    ...
    // Set CS decoder controls
    if (DIFF_FORCED(DoDecode)){
        if (pCFG->DoDecode)
            SRS_CSHP_SetRenderingMode(Obj, SRS_CSHP_CS_DECODER);
        else
            SRS_CSHP_SetRenderingMode(Obj, SRS_CSHP_PASSIVE_DECODER);
    }

    if (DIFF_FORCED(DecodeMode)){
        if (pCFG->DecodeMode == 0)
            SRS_CSHP_SetCSDecoderProcMode(Obj, SRS_CSD_CINEMA);
        else
            SRS_CSHP_SetCSDecoderProcMode(Obj, SRS_CSD_MUSIC);
    }

    // Set TBHD controls
    if (DIFF_FORCED(DoTB)) SRS_CSHP_SetTruBassEnable(Obj, pCFG->DoTB);
    ...
    if (DIFF_FORCED(TBCompress)) SRS_CSHP_SetTruBassCompressorLevel(Obj, SRS_FXP16(Tool_MaxZero(pCFG->TBCompress, 1.0f), SRS_TBHD_CTRL_IWL));

    if (DIFF_FORCED(TBMode)){
        if (pCFG->TBMode == 0)
            SRS_CSHP_SetTruBassMode(Obj, SRS_TBHD_MODE_MONO);
        else
            SRS_CSHP_SetTruBassMode(Obj, SRS_TBHD_MODE_STEREO);
    }
    ...
    if (DIFF_FORCED(TBFreq) || DIFF_FORCED(TBAnalyze)){
        ...
        SRS_CSHP_SetTruBassSpeakerSize(Obj, tSetCustom);
        SRS_CSHP_SetTruBassCustomSpeakerFilterCoefs(Obj, TBCoefs);
    }

    if (DIFF_FORCED(DoTBSplit)) SRS_CSHP_SetTruBassSplitAnalysisEnable(Obj, pCFG->DoTBSplit);

    if (DIFF_FORCED(DoDef)) SRS_CSHP_SetDefinitionEnable(Obj, pCFG->DoDef);
    if (DIFF_FORCED(DefMin) || DIFF_FORCED(DefWindow) || DIFF_FORCED(DefSlide)){
        ...
        //SRS_CSHP_SetDefinitionLevel(Obj, SRS_FXP16(DFWant, SRS_DEFINITION_IWL));
        if (ForceActive) DFActive = -1.0f;
    }

    // Set Dialog controls
    if (DIFF_FORCED(DoDialog))  SRS_CSHP_SetDialogClarityEnable(Obj, pCFG->DoDialog);
    if (DIFF_FORCED(Dialog))    SRS_CSHP_SetDialogClarityLevel(Obj, SRS_FXP16(Tool_MaxZero(pCFG->Dialog, 1.0f), SRS_FOCUS_IWL));

    // Set Limiter controls
    if (DIFF_FORCED(DoLimit))   SRS_CSHP_SetLimiterEnable(Obj, pCFG->DoLimit);
    if (DIFF_FORCED(LimitMGain))SRS_CSHP_SetLimiterMinimalGain(Obj, SRS_FXP32(pCFG->LimitMGain, 2));

    if (DIFF_FORCED(IGain)) SRS_CSHP_SetInputGain(Obj, SRS_FXP16(Tool_MaxZero(pCFG->IGain, 1.0f), SRS_CSHP_GAIN_IWL));
    if (DIFF_FORCED(OGain)) SRS_CSHP_SetOutputGain(Obj, SRS_FXP16(Tool_MaxZero(pCFG->OGain, 1.0f), SRS_CSHP_GAIN_IWL));
    if (DIFF_FORCED(BGain)) SRS_CSHP_SetBypassGain(Obj, SRS_FXP16(Tool_MaxZero(pCFG->BGain, 1.0f), SRS_CSHP_GAIN_IWL));

    if (bBypass == false)
        SRS_CSHP_SetEnable(Obj, true);
    else
        SRS_CSHP_SetEnable(Obj, false);
...
void SRS_Source_CSHP::Process_256(SRSSamp *pData){
    if (TBWant != TBActive){    // Slide into requested TB value?
        ...
        SRS_CSHP_SetTruBassLevel(Obj, SRS_FXP16(TBActive, SRS_TBHD_CTRL_IWL));
    }
    ...
    if (DFWant != DFActive){    // Slide into requested Def value?
        ...
        SRS_CSHP_SetDefinitionLevel(Obj, SRS_FXP16(DFActive, SRS_DEF_IWL));
    }
    ...
    SRS_CSHP_Process(Obj, &inChans, 256, pObjWorkspace);
}
...
char* SRS_GetVersion_CSHP(char* pWork, size_t maxBytes){
    int v1 = SRS_CSHP_GetTechVersion(SRS_VERSION_MAJOR);
    int v2 = SRS_CSHP_GetTechVersion(SRS_VERSION_MINOR);
    int v3 = SRS_CSHP_GetTechVersion(SRS_VERSION_REVISION);
    int v4 = SRS_CSHP_GetTechVersion(SRS_VERSION_RELEASE);
    int l1 = SRS_CSHP_GetLibVersion(SRS_VERSION_MAJOR);
    int l2 = SRS_CSHP_GetLibVersion(SRS_VERSION_MINOR);
    int l3 = SRS_CSHP_GetLibVersion(SRS_VERSION_REVISION);
    int l4 = SRS_CSHP_GetLibVersion(SRS_VERSION_RELEASE);
