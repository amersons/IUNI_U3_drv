...
const char* gInfo_LibVers = "v3.0.0.8"
...
const char* gInfo_IPs = "cshp,trueq,hlimit,hpf"
...
SRS_Param gCFG_Params[] = {
    ...
    { CFG_Lib_Integration,  SRS_PTYP_INFO,  SRS_PFMT_STATIC,    0.0f,   0.0f,   1.0f,   "lib_integration", "Library Integration", "v2.3.3.0", "", 0},
...
const int32_t gProcStep = 256/(PROC_FADELEN-PROC_DELAYIN);
void FadeLoop_2Chan(sample_t* pOut, sample_t* pS0, sample_t* pS1, int32_t tween, int32_t step, int steps){
    ...
    for (i=0; i<steps; i++, tween+=step, pOut+=2, pS0+=2, pS1+=2){
        ...
        int32_t tL = (float)pS0[0] / 256 * tTw + (float)pS1[0] / 256 * (256 - tTw);
        int32_t tR = (float)pS0[1] / 256 * tTw + (float)pS1[1] / 256 * (256 - tTw);
...
#ifdef _SRSCFG_ARCH_ARM
#ifdef _SRS_PROC_32BIT
                *pCInL++ = ((int64_t)*pSampsIn++*ActiveGain) >> 8;
                *pCInR++ = ((int64_t)*pSampsIn++*ActiveGain) >> 8;
                *pSampsOut++ = *pCOutL++;
                *pSampsOut++ = *pCOutR++;
#else
...
#ifdef _SRSCFG_ARCH_ARM
#ifdef _SRS_PROC_32BIT
                *pInL++ = ((int64_t)*pSampsIn++*ActiveGain)>>8;
                *pInR++ = ((int64_t)*pSampsIn++*ActiveGain)>>8;
#else
