...
#ifdef CONFIG_GN_Q_BSP_HIFI_24BITS
void adev_set_hifi(struct audio_device *adev, int val);
/* #define AUDIO_HW_PCM_16BITS */
/* #define AUDIO_HW_PCM_24BITS */
#define AUDIO_HW_PCM_32BITS
#define USECASE_AUDIO_PLAYBACK_PRIMARY_HIFI USECASE_AUDIO_PLAYBACK_DEEP_BUFFER_HIFI
#endif
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#if (defined CONFIG_GN_Q_BSP_HIFI_24BITS)
#if (defined AUDIO_HW_PCM_32BITS)
    .format = PCM_FORMAT_S32_LE,
#elif (defined AUDIO_HW_PCM_24BITS)
    .format = PCM_FORMAT_S24_LE,
#elif (defined AUDIO_HW_PCM_16BITS)
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef CONFIG_GN_Q_BSP_HIFI_24BITS
	[USECASE_AUDIO_PLAYBACK_DEEP_BUFFER_HIFI] = "deep-buffer-playback-hifi",
#endif
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI end
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef CONFIG_GN_Q_BSP_HIFI_24BITS
			if(!((usecase->stream.out->devices & AUDIO_DEVICE_OUT_WIRED_HEADPHONE)
			   ||(usecase->stream.out->devices & AUDIO_DEVICE_OUT_WIRED_HEADSET))){
			ALOGI("Headset is out,turn HIFI off.");
			adev_set_hifi(adev, 0);
			}
#endif
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI end
            usecase->devices = usecase->stream.out->devices;
...
#ifdef CONFIG_GN_Q_BSP_HIFI_24BITS
	if(hifi_on){
			out->usecase = USECASE_AUDIO_PLAYBACK_PRIMARY_HIFI;
	}
#endif
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI end
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#if (defined CONFIG_GN_Q_BSP_HIFI_24BITS)
#ifdef GIONEE_AUDIO_HW_DUMP_PCM_DATA
			FILE *fp;
			if ((fp = fopen("/data/hw0.cap", "a+")) != NULL) {
				fwrite((uint8_t *)buffer, 1, bytes, fp);
				fclose(fp);
			}else{
				ALOGE("hw0.cap OPEN FAILED");
			}
#endif
#if (defined  AUDIO_HW_PCM_32BITS)
			 ALOGVV("%s: writing buffer (%d bytes) to pcm device,period_size:%d,framesize:%d",
				   __func__, bytes, out->config.period_size, audio_stream_frame_size(stream));
		    ret = pcm_write(out->pcm, (void *)buffer, bytes);
            if (ret == 0)
                out->written += bytes / (out->config.channels * sizeof(int));
#elif (defined AUDIO_HW_PCM_24BITS)
			int *src = buffer;
			int *dst = buffer;
			int i;
...
#ifdef GIONEE_AUDIO_HW_DUMP_AUDIO_HW_PCM_DATA
			if ((fp = fopen("/data/hw.cap", "a+")) != NULL) {
				fwrite((uint8_t *)buffer, 1, bytes / 4 *3, fp);
				fclose(fp);
			}else{
				ALOGE("hw.cap OPEN FAILED");
			}
#endif
            ret = pcm_write(out->pcm, (void *)buffer, bytes / 4 * 3 );
            if (ret == 0)
                out->written += bytes / (out->config.channels * 3);
#elif (defined  AUDIO_HW_PCM_16BITS)
			int *src = buffer;
			short *dst = buffer;
			int i;
...
#ifdef GIONEE_AUDIO_HW_DUMP_PCM_DATA
			if ((fp = fopen("/data/hw.cap", "a+")) != NULL) {
				fwrite((uint8_t *)buffer, 1, bytes / 2, fp);
				fclose(fp);
			}else{
				ALOGE("hw.cap OPEN FAILED");
			}
#endif
            ret = pcm_write(out->pcm, (void *)buffer, bytes / 2);
            if (ret == 0)
...
#ifdef CONFIG_GN_Q_BSP_HIFI_24BITS
void adev_set_hifi(struct audio_device *adev, int val){
    struct audio_usecase *usecase_slim = NULL;
    struct audio_usecase *usecase_i2s = NULL;
    usecase_i2s = get_usecase_from_list(adev, USECASE_AUDIO_PLAYBACK_PRIMARY_HIFI);
    usecase_slim = get_usecase_from_list(adev, USECASE_AUDIO_PLAYBACK_PRIMARY);
    if(val){
        hifi_on = 1;
        if(usecase_slim){
            disable_audio_route(adev,usecase_slim);
            usecase_slim->id = USECASE_AUDIO_PLAYBACK_PRIMARY_HIFI;
            enable_audio_route(adev,usecase_slim);
         }
     }else{
        hifi_on = 0;
        if(usecase_i2s){
            disable_audio_route(adev, usecase_i2s);
            usecase_i2s->id = USECASE_AUDIO_PLAYBACK_PRIMARY;
            enable_audio_route(adev, usecase_i2s);
        }
      }
}
#endif
...
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI begin
#ifdef CONFIG_GN_Q_BSP_HIFI_24BITS
	err = str_parms_get_str(parms, "HIFI_SWITCH", value, sizeof(value));
    if (err >= 0) {
        val = atoi(value);
		ALOGV("HIFI status:%d",val);
        adev_set_hifi(adev, val);
	}
#endif
//GIONEE guogy 2014-10-31 CR01406672 modify for 24bits HIFI end
...
//Gionee huangzhuolin 20140506 add for mic mmitest by CR01236873 begin
#ifdef CONFIG_GN_Q_BSP_AUDIO_MMI_TEST_SUPPORT
    err = str_parms_get_int(parms, "MMIMic", &val);
    if (err >= 0) {
        ALOGE("%s: MMIMIC = = = = = = %d", __func__, val);
		switch(val) {
			case 1:
				adev->mmi_test_mic = 1;
				break;
			...
			case 4:
				adev->mmi_test_mic = 3;
				break;
//Gionee huangzhuolin 20140725 add for autommi test by CR01334214 begin
			case 5:
				adev->mmi_test_mic = 4;
				break;
//Gionee huangzhuolin 20140725 add for autommi test by CR01334214 end
			default:
				adev->mmi_test_mic = 0;
				break;
		}
...
#endif
//Gionee huangzhuolin 20140506 add for mic mmitest by CR01236873 end
...
#ifdef CONFIG_GN_Q_BSP_HIFI_24BITS
    ALOGV("keys:%s", keys);
	if(!strcmp(keys, "HIFI_SWITCH")){
		ALOGE("keys:%s,hifi_on:%d", keys, hifi_on);
		pthread_mutex_unlock(&adev->lock);
		if(hifi_on)
			return strdup("1");
		else
