		...
		const char *buf, size_t size)
{
	...
	printk("%s, %d\n", __func__, wake_switch);
       ...
       if (2 == wake_switch) {
           ...
           printk("liudj tpd: test_wake_flag ====> %d\n", test_wake_flag);
       }
...
static int wake_tp_create_attr(struct device *dev) 
{
	...
	for(idx = 0; idx < num; idx++)
	{
		...
		{            
			printk("tpd liudj: device_create_file (%s) = %d\n", wake_tp_attr_list[idx]->attr.name, err);        
			break;
		}
	}
...
static int synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    u16 swipe_distance_y;
       ...
       if (init_not_complete) {
            printk("liudj: tpd sensor_report, but init_not_complete = 0, so return\n");
            return 0;
       }
#endif
...
static int synaptics_rmi4_query_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
       ...
       if (retval < 0) {
           printk("liudj tpd: read product id failed...\n");
       }else {
           printk("liudj tpd: Product id is %s\n", product_id);
       }

       //Read firmware ID
       retval = synaptics_rmi4_i2c_read(rmi4_data, 0x4d, firmware_id, 4);
       if (retval < 0) {
           printk("liudj tpd: read firmware id failed...\n");
       }else {
           printk("liudj tpd: Firmware id is 0x%02x 0x%02x 0x%02x 0x%02x\n", firmware_id[0], firmware_id[1], firmware_id[2], firmware_id[3]);
       }
...
static int synaptics_rmi4_reset_command(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		...
		if (done) {
			dev_info(&rmi4_data->i2c_client->dev,
				"liudj tpd %s: Find F01 in page description table 0x%x\n",
	...
	if (!done) {
		dev_err(&rmi4_data->i2c_client->dev,
			"liudj tpd %s: Cannot find F01 in page description table\n",
			__func__);
		return -EINVAL;
	}
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"liudj tpd %s: Failed to issue reset command, error = %d\n",
				__func__, retval);
		return retval;
	}
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"liudj --> tpd %s: Failed to query device\n",
				__func__);
		return retval;
	}
...
static void gn_resume_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	struct synaptics_rmi4_fn *fhandler;
       ...
       printk("tpd liudj: init_not_complete = %d\n", init_not_complete);
    
       if (init_not_complete) {
            retval = synaptics_rmi4_query_device(rmi4_data);
            if (retval < 0)
                return;
            printk("tpd liudj: query device success\n");
     ...
     input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_F22);
	     ...
	     if (retval) {
		  ...
		  if (retval < 0) {
            ...
            init_not_complete = 0;
            printk("liudj: tpd set init_not_complete = 0\n");
       }
}
#endif
...
static void gn_esd_reset_work(struct work_struct *work)
{
    ...
    printk("liudj tpd: gn_esd_reset_work --> reset TP\n");
}
#endif
...
static void gn_tp_test_work(struct work_struct *work)
{
    ...
    if (retval < 0) {
        printk("liudj tpd: read int_test_flag (0x0014) failed...\n");
    }
    printk("liudj tpd: int_test_flag = %d\n", int_test_flag);
...
int gn_tp_reset(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    synaptics_rmi4_power_on(rmi4_data, true);
    gpio_direction_output(rmi4_data->board->en_gpio, 1);
    printk("liudj tpd: gn_tp_reset --> reset TP Compelete\n");
    return 0;
}
...
static void synaptics_rmi4_double_wakeup_enter(struct synaptics_rmi4_data *rmi4_data)
{
	...
	unsigned char device_ctrl, button_ctrl;
       ...
       printk("liudj: enter double wakeup...\n");
       ...
       retval = synaptics_rmi4_i2c_write(rmi4_data,
			...
			sizeof(button_ctrl));
       if (retval < 0) {
           printk("liudj tpd: Disable 0D button Failed...\n");
       }
	...
	if (retval < 0) {
		dev_err(&(rmi4_data->input_dev->dev),
				"liudj: %s: Failed to enter sleep mode\n",
				__func__);
		rmi4_data->sensor_sleep = false;
		return;
	} else {
       ...
       printk("liudj: 0x57 register = 0x%x\n", device_ctrl);
 ...
 static void synaptics_rmi4_double_wakeup_exit(struct synaptics_rmi4_data *rmi4_data)
{
	...
	unsigned char device_ctrl, button_ctrl, int_test = 0;
       ...
       printk("liudj tpd: double wakeup exit...\n");
       ...
       do {
	    ...
	    if (retval < 0) {
           ...
           printk("liudj tpd: 0x57 register = 0x%x\n", device_ctrl);
       ...
       retval = synaptics_rmi4_i2c_write(rmi4_data,
			...
			sizeof(button_ctrl));
       if (retval < 0) {
           printk("liudj tpd: Enable 0D button Failed...\n");
       }

       synaptics_rmi4_i2c_read(rmi4_data, 0x0014, &int_test, 1);
       printk("liudj tpd: read int_test = 0x%x\n", int_test);
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	int count = 0;
...
#ifdef INIT_TP_WHEN_RESUME
       if (init_not_complete) {
            printk("liudj: TP init not complete, suspend return 0\n");
            return 0;
       }
#endif
       ...
       if (wake_switch || gesture_switch) {
           printk("tpd liudj: synaptics_rmi4_suspend --> rmi4_data->irq = %d\n", rmi4_data->irq);
...
#ifdef DOUBLE_CLICK_WAKE
       if (wake_suspend_compelete) {
           printk("liudj tpd: double wake resume...\n");
