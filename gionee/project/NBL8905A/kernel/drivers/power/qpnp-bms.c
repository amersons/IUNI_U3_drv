...
/*****moxizheng add for,CR01111512********/
static int gn_get_battery_status(void)
{
	union power_supply_propval ret = {0,};
	...
	if (batt_psy) {
        ...
        if( (ret.intval==POWER_SUPPLY_HEALTH_OVERHEAT)||(ret.intval==POWER_SUPPLY_HEALTH_COLD) )
        {        
            pr_debug(" over heat or cold\n");
            return 1;
        }
	}
...
//Gionee wudp 2014-05-04 add for optimize bms soc begin
		...
		if(abs(soc - shutdown_soc) > 20){  //gionee yangshi 20141106 modify for CR01404747
			printk(KERN_INFO "first_time_calc_soc soc = %d, shutdown_soc = %d", soc, shutdown_soc);
			if(get_battery_status(chip) != POWER_SUPPLY_STATUS_CHARGING) //gionee yangshi 20141110 add for CR01408948
				shutdown_soc = soc;
		}
#endif
...
done_calculating:
	...
	if ( ((new_calculated_soc != previous_soc) || gn_get_battery_status() )&& chip->bms_psy_registered) {//moxizheng add for,CR01111512
		power_supply_changed(&chip->bms_psy);
