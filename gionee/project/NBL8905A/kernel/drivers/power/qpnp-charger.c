...
/* Register 00h */
#define BQ2419X_INPUT_SRC_CTRL		0x00
#define BQ2419X_VINDPM_MASK		0x78
#define BQ2419X_VINDPM_SHIFT		3
#define BQ2419X_IINLIM_MASK		0x07
#define BQ2419X_IINLIM_SHIFT		0

/* Register 01h */
#define BQ2419X_POC_CTRL		0x01
#define BQ2419X_CC_MASK		    0x30
#define BQ2419X_CC_SHIFT		4
#define BQ2419X_WD_RST_MASK		0x40
#define BQ2419X_WD_RST_SHIFT		6
#define BQ2419X_WD_RST_ENABLE		0x01
#define BQ2419X_CC_DISABLE		0x00
#define BQ2419X_CC_ENABLE		0x01
#define BQ2419X_CC_OTG  		0x10



/* Register 02h */
#define BQ2419X_ICHG_CTRL		0x02
#define BQ2419X_ICHG_MASK		0xFC
#define BQ2419X_ICHG_SHIFT		2

/* Register 04h */
#define BQ2419X_VREG_CTRL		0x04
#define BQ2419X_VREG_MASK		0xFC
#define BQ2419X_VREG_SHIFT		2

/* Register 05h */
#define BQ2419X_WD_CTRL			0x05
#define BQ2419X_WD_MASK			0x30
#define BQ2419X_WD_SHIFT		4
#define BQ2419X_WD_DISABLE		0x00
#define BQ2419X_WD_80S			0x02

/* Register 08h */
#define BQ2419X_STATUS			0x08
#define BQ2419X_VBUS_STATUS_MASK	0xC0
#define BQ2419X_VBUS_STATUS_SHIFT	6
#define BQ2419X_CHG_STAT_MASK		0x30
#define BQ2419X_CHG_STAT_SHIFT		4
#define BQ2419X_NOT_CHARGING		0
#define BQ2419X_FAST_CHARGING		2
#define BQ2419X_CHARGE_DONE		3

/* Register 09h */
#define BQ2419X_FAULT			0x09

/* Charging parameters */
#define BQ2419X_DEFAULT_USB_VLIM	BQ2419X_VLIM_4360mV
#define	BQ2419X_DEFAULT_USB_ILIM	BQ2419X_ILIM_500mA
#define	BQ2419X_DEFAULT_USB_ICHG	BQ2419X_ICHG_512mA
#define	BQ2419X_DEFAULT_USB_VREG	BQ2419X_VREG_4352mV
//yangshi 20141119 add for CR01414190 begin
#define BQ2419X_DEFAULT_CT_VLIM		BQ2419X_VLIM_4360mV
#define	BQ2419X_DEFAULT_CT_ILIM		BQ2419X_ILIM_900mA
#define	BQ2419X_DEFAULT_CT_ICHG		BQ2419X_ICHG_896mA
#define	BQ2419X_DEFAULT_CT_VREG		BQ2419X_VREG_4352mV
//yangshi 20141119 add for CR01414190 end

#define BQ2419X_DEFAULT_TA_VLIM		BQ2419X_VLIM_4360mV
#define BQ2419X_DEFAULT_TA_ILIM		BQ2419X_ILIM_3000mA
#define BQ2419X_DEFAULT_TA_ICHG		BQ2419X_ICHG_2048mA
#define	BQ2419X_DEFAULT_TA_VREG		BQ2419X_VREG_4352mV

enum bq2419x_vbus_type {
	BQ2419X_VBUS_NONE,
	BQ2419X_VBUS_USB_CHARGER,
	BQ2419X_VBUS_CT_CHARGER, 
	BQ2419X_VBUS_TA_CHARGER,
	BQ2419X_VBUS_OTG,
};
...
struct bq2419x {
	...
	/* Charging parameters */
	struct bq2419x_charge_param usb_param;
	struct bq2419x_charge_param ct_param;
	struct bq2419x_charge_param ta_param;

    //dengyj CR01388955 begin
    int charger_type;
	struct wake_lock charger_wake_lock;
    //dengyj CR01388955 end

    //dengyj CR01389404 begin
	u8	bq2419_usbin_health;
    int bq2419_batt_health;
	bool  bq2419_charging_disabled;
    //dengyj CR01389404 end

};

struct bq2419x *gn_bq2419x = NULL;//dengyj CR01388955

//dengyj CR01389404 begin
extern struct qpnp_chg_chip *g_chip;
static int qpnp_chg_check_usbin_health(struct qpnp_chg_chip *chip);
static int get_prop_batt_health(struct qpnp_chg_chip *chip);
//dengyj CR01389404 end

static int bq2419x_disable_watchdog_timer(struct bq2419x *bq);

static int bq2419x_reset_watchdog_timer(struct bq2419x *bq);

static int bq2419x_init_watchdog_timer(struct bq2419x *bq);
#endif
...
struct qpnp_chg_chip *g_chip = NULL;//dengyj CR01389404

static int bq2419x_read_byte(struct bq2419x *bq, u8 *data, u8 reg)
{
	int ret;
...
static int bq2419x_write_byte(struct bq2419x *bq, u8 reg, u8 data)
{
    int ret;
...
static int bq2419x_update_bits(struct bq2419x *bq, u8 reg, u8 mask, u8 data)
{
	int ret;
	u8 tmp;

	ret = bq2419x_read_byte(bq, &tmp, reg);
	if (ret)
		return ret;
	...
	return bq2419x_write_byte(bq, reg, tmp);
}
/*

static int bq2419x_disable_charge(void)//moxizheng add
{

 return bq2419x_update_bits(gn_bq2419x, BQ2419X_POC_CTRL, BQ2419X_CC_MASK, BQ2419X_CC_DISABLE << BQ2419X_CC_SHIFT);

}

static int bq2419x_enble_charge(void)//moxizheng add
{

return bq2419x_update_bits(gn_bq2419x, BQ2419X_POC_CTRL, BQ2419X_CC_MASK, BQ2419X_CC_ENABLE << BQ2419X_CC_SHIFT);
...
#endif
static enum bq2419x_vbus_type bq2419x_get_vbus_type(struct bq2419x *bq)
{

	u8 val = 0;
    //dengyj CR01388955 begin
/*
	bq2419x_read_byte(bq, &val, BQ2419X_STATUS);
	val &= BQ2419X_VBUS_STATUS_MASK;
	val >>= BQ2419X_VBUS_STATUS_SHIFT;
    */
  if(bq)
  {
      return bq->charger_type;
  }
  //dengyj CR01388955 end
...
static int bq2419x_charge_status(struct bq2419x *bq)
{
	u8 val = 0;

	bq2419x_read_byte(bq, &val, BQ2419X_STATUS);
	val &= BQ2419X_CHG_STAT_MASK;
	val >>= BQ2419X_CHG_STAT_SHIFT;
...
static int bq2419x_charge_type(struct bq2419x *bq)
{
        u8 val = 0;
...
{
	struct bq2419x *bq = container_of(psy, struct bq2419x, usb);

	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		if (bq2419x_get_vbus_type(bq) == BQ2419X_VBUS_USB_CHARGER)
			val->intval = 1;
		else
			val->intval = 0;
		break;
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		val->intval = bq2419x_charge_status(bq);
		break;
	default:
		return -EINVAL;
	}
...
static int bq2419x_wall_get_property(struct power_supply *psy,
				...
				union power_supply_propval *val)
{
	struct bq2419x *bq = container_of(psy, struct bq2419x, wall);

	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		if (bq2419x_get_vbus_type(bq) == BQ2419X_VBUS_TA_CHARGER)
			val->intval = 1;
		else
			val->intval = 0;
		break;
...
//dengyj CR01389404 begin
struct bq2419x *get_bq2419(void)
{
    return gn_bq2419x;
}

struct qpnp_chg_chip *get_chip(void)
{
    return g_chip;
}

static void dump_bq2419(void)
{
	u8 addr;
	u8 val;
    struct bq2419x *bq = get_bq2419();

    if(bq)
    {
        pr_debug("\nbq2419 reg :: ");
	    for (addr = 0x0; addr <= 0x0A; addr++) {
		    bq2419x_read_byte(bq, &val, addr);
		    pr_debug("[0x%.2x] = 0x%.2x , ", addr, val);
	    }
        pr_debug("\n");
    }
...
static int bq2419x_charger_enable(bool enable)
{
    int ret = -1;
    ...
    if(get_bq2419())
    {
        if(enable)
        {
            ret = bq2419x_update_bits(get_bq2419(), BQ2419X_POC_CTRL, BQ2419X_CC_MASK, BQ2419X_CC_ENABLE << BQ2419X_CC_SHIFT);
        }
        else
        {
            ret = bq2419x_update_bits(get_bq2419(), BQ2419X_POC_CTRL, BQ2419X_CC_MASK, BQ2419X_CC_DISABLE << BQ2419X_CC_SHIFT);
...
static int bq2419x_update_charge_params(struct bq2419x *bq)
{
	enum bq2419x_vbus_type type = bq2419x_get_vbus_type(bq);
	int ret;
	u8 vlim;
	u8 ilim;
	u8 ichg;
	...
	case BQ2419X_VBUS_USB_CHARGER:
		...
		break;
//yangshi 20141119 add for CR01414190 begin
	case BQ2419X_VBUS_CT_CHARGER:
		vlim = bq->ct_param.vlim;
		ilim = bq->ct_param.ilim;
		ichg = bq->ct_param.ichg;
		vreg = bq->ct_param.vreg;
		break;
//yangshi 20141119 add for CR01414190 end
	case BQ2419X_VBUS_TA_CHARGER:
    ...
    pr_debug("bq2419 charger type:%d\n",type);//dengyj CR01388955

	ret = bq2419x_update_bits(bq, BQ2419X_INPUT_SRC_CTRL,
			BQ2419X_VINDPM_MASK, vlim << BQ2419X_VINDPM_SHIFT);
	if (ret)
		return ret;

	ret = bq2419x_update_bits(bq, BQ2419X_INPUT_SRC_CTRL,
			BQ2419X_IINLIM_MASK, ilim << BQ2419X_IINLIM_SHIFT);
	if (ret)
		return ret;

	ret = bq2419x_update_bits(bq, BQ2419X_ICHG_CTRL,
			BQ2419X_ICHG_MASK, ichg << BQ2419X_ICHG_SHIFT);
	if (ret)
		return ret;

	return bq2419x_update_bits(bq, BQ2419X_VREG_CTRL,
			BQ2419X_VREG_MASK, vreg << BQ2419X_VREG_SHIFT);
}


static void bq2419x_irq_workfunc(struct work_struct *work)
{
	struct bq2419x *bq = container_of(work, struct bq2419x, irq_work);
	u8 status;
	u8 fault;
	int ret;

	/* Disable the watchdog timer forcedly */
	bq2419x_disable_watchdog_timer(bq);

	/* Read STATUS and FAULT registers */
	ret = bq2419x_read_byte(bq, &status, BQ2419X_STATUS);
	if (ret) {
		dev_err(bq->dev, "i2c failure:%d\n", ret);
		return;
	}

	ret = bq2419x_read_byte(bq, &fault, BQ2419X_FAULT);
	if (ret) {
		dev_err(bq->dev, "i2c failure: %d\n", ret);
		return;
	}
	...
	bq2419x_update_charge_params(bq);
}
#define BQ2419X_CHARGER_MS 10000//80000 dengyj CR01388955
static void bq2419x_charger_workfunc(struct work_struct *work)
{
        //dengyj CR01389404 begin
        struct bq2419x *bq = get_bq2419();
	    u8	bq2419_usbin_health;
        ...
        /* Reset the watchdog timer forcedly */
        bq2419x_reset_watchdog_timer(bq);
        ...
        if(get_chip())
        {
		    bq2419_usbin_health = qpnp_chg_check_usbin_health(get_chip());
        	if (bq->bq2419_usbin_health != bq2419_usbin_health) {
		    bq->bq2419_usbin_health = bq2419_usbin_health;
		    if (bq2419_usbin_health == USBIN_OVP)
			    psy_health_sts = POWER_SUPPLY_HEALTH_OVERVOLTAGE;
		    else if (bq2419_usbin_health == USBIN_OK)
			    psy_health_sts = POWER_SUPPLY_HEALTH_GOOD;
		    power_supply_set_health_state(get_chip()->usb_psy, psy_health_sts);
		    power_supply_changed(get_chip()->usb_psy);
	       }

            bq2419_batt_health = get_prop_batt_health(get_chip());
            pr_info("bq2419 battery health:%d\n",bq2419_batt_health);
        	if (bq->bq2419_batt_health != bq2419_batt_health) {
		    bq->bq2419_batt_health = bq2419_batt_health;
		    power_supply_changed(get_chip()->usb_psy);
	       }

        }
        if(bq->bq2419_usbin_health == USBIN_OVP
           ||bq->bq2419_batt_health == POWER_SUPPLY_HEALTH_OVERHEAT
           ||bq->bq2419_batt_health == POWER_SUPPLY_HEALTH_COLD)
        {
            bq2419x_charger_enable(false);
        }
        else
        {
            bq2419x_charger_enable(true);
        }
        //dengyj CR01389404 end

	schedule_delayed_work(&bq->charger_work,
		msecs_to_jiffies(BQ2419X_CHARGER_MS));
}


#if 0

static irqreturn_t bq24196_interrupt(int irq, void *data)
{
	struct bq2419x *bq = data;
...
static int bq2419x_disable_watchdog_timer(struct bq2419x *bq)
{
	u8 val = BQ2419X_WD_DISABLE << BQ2419X_WD_SHIFT;

	return bq2419x_update_bits(bq, BQ2419X_WD_CTRL, BQ2419X_WD_MASK, val);
}

static int bq2419x_reset_watchdog_timer(struct bq2419x *bq)
{
        u8 val = BQ2419X_WD_RST_ENABLE << BQ2419X_WD_RST_SHIFT;

        return bq2419x_update_bits(bq, BQ2419X_POC_CTRL, BQ2419X_WD_RST_MASK, val);
}

static int bq2419x_init_watchdog_timer(struct bq2419x *bq)
{
        u8 val = BQ2419X_WD_80S << BQ2419X_WD_SHIFT;

        return bq2419x_update_bits(bq, BQ2419X_WD_CTRL, BQ2419X_WD_MASK, val);
}

static int bq2419x_init_charge_params(struct bq2419x *bq,
				struct bq2419x_platform_data *pdata)
{
	 ...
	 */
	bq->usb_param.vlim = BQ2419X_DEFAULT_USB_VLIM;
	bq->usb_param.ilim = BQ2419X_DEFAULT_USB_ILIM;
	bq->usb_param.ichg = BQ2419X_DEFAULT_USB_ICHG;
	bq->usb_param.vreg = BQ2419X_DEFAULT_USB_VREG;

//yangshi 20141119 add for CR01414190 begin
	bq->ct_param.vlim = BQ2419X_DEFAULT_CT_VLIM;
	bq->ct_param.ilim = BQ2419X_DEFAULT_CT_ILIM;
	bq->ct_param.ichg = BQ2419X_DEFAULT_CT_ICHG;
	bq->ct_param.vreg = BQ2419X_DEFAULT_CT_VREG;
//yangshi 20141119 add for CR01414190 end

	bq->ta_param.vlim = BQ2419X_DEFAULT_TA_VLIM;
	bq->ta_param.ilim = BQ2419X_DEFAULT_TA_ILIM;
	bq->ta_param.ichg = BQ2419X_DEFAULT_TA_ICHG;
	bq->ta_param.vreg = BQ2419X_DEFAULT_TA_VREG;
...
//yangshi 20141119 add for CR01414190 begin
	if (pdata->ct_param) {
		bq->ct_param.vlim = pdata->ct_param->vlim;
		bq->ct_param.ilim = pdata->ct_param->ilim;
		bq->ct_param.ichg = pdata->ct_param->ichg;
		bq->ct_param.vreg = pdata->ct_param->vreg;
	}
//yangshi 20141119 add for CR01414190 end
...
static int bq2419x_init_device(struct bq2419x *bq,
			struct bq2419x_platform_data *pdata)
{
	int ret;
	...
	ret = bq2419x_init_watchdog_timer(bq);
	if (ret)
		return ret;

	return bq2419x_init_charge_params(bq, pdata);
}
/*
static int bq2419x_psy_register(struct bq2419x *bq)
{
	int ret;

	bq->usb.name = "bq2419x-usb";
	bq->usb.type = POWER_SUPPLY_TYPE_USB;
	bq->usb.properties = bq2419x_charger_props;
	bq->usb.num_properties = ARRAY_SIZE(bq2419x_charger_props);
	bq->usb.get_property = bq2419x_usb_get_property;
	...
	bq->wall.name = "bq2419x-wall";
	bq->wall.type = POWER_SUPPLY_TYPE_MAINS;
	bq->wall.properties = bq2419x_charger_props;
	bq->wall.num_properties = ARRAY_SIZE(bq2419x_charger_props);
	bq->wall.get_property = bq2419x_wall_get_property;
...
static void bq2419x_psy_unregister(struct bq2419x *bq)
{
...
static ssize_t bq2419x_show_registers(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct bq2419x *bq = dev_get_drvdata(dev);
	u8 addr;
	u8 val;
...
static ssize_t bq2419x_store_registers(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	unsigned long addr = simple_strtoul(buf, &after, 16);
	
	pr_info("bq2419 reg=0x%lx\n",addr);
	pr_info("bq2419 after =%s\n",after);
 	while (*after != '\0' && (*after==' ')) 
 		after++;
 	addr_value = simple_strtoul(after, NULL, 16);
	pr_info("bq2419 value=0x%lx\n",addr_value);

    if(get_bq2419())
    {
	    bq2419x_write_byte(get_bq2419(),addr, addr_value);
    }
    //dengyj CR01389404 end
...
static DEVICE_ATTR(registers, S_IRUGO|S_IWUGO, bq2419x_show_registers, bq2419x_store_registers);
			   ...
			   const struct i2c_device_id *id)
{
	...
	int ret;
    pr_info("bq2419 charger_probe....\n");
	bq = kzalloc(sizeof(struct bq2419x), GFP_KERNEL);
	if (!bq)
		return -ENOMEM;
	...
	ret = bq2419x_init_device(bq, pdata);
	if (ret) {
		dev_err(bq->dev, "device init failure: %d\n", ret);
		goto err_dev;
	}

	INIT_WORK(&bq->irq_work, bq2419x_irq_workfunc);
	INIT_DELAYED_WORK(&bq->charger_work, bq2419x_charger_workfunc);
/*
	ret = bq2419x_psy_register(bq);
	if (ret)
		goto err_dev;
*/
	ret = sysfs_create_group(&bq->dev->kobj, &bq2419x_attr_group);
	if (ret) {
		dev_err(bq->dev, "failed to register sysfs. err: %d\n", ret);
		goto err_dev;
	}
/*
	ret = request_irq(bq->irq, bq24196_interrupt,
...
*/
    ...
    //dengyj CR01389404 begin
    bq->bq2419_charging_disabled = false;
    bq->bq2419_usbin_health = USBIN_UNKNOW;
    bq->bq2419_batt_health = POWER_SUPPLY_HEALTH_UNKNOWN;
    //dengyj CR01389404 end

    //dengyj CR01388955 begin
    bq->charger_type = BQ2419X_VBUS_NONE;
	wake_lock_init(&bq->charger_wake_lock, WAKE_LOCK_SUSPEND,
			"charger_wake_lock");
    //dengyj CR01388955 end

	pr_info("bq2419 %s,sucess\n",__FUNCTION__);
	schedule_work(&bq->irq_work);//moxizheng add
        schedule_delayed_work(&bq->charger_work,
                msecs_to_jiffies(BQ2419X_CHARGER_MS));
...
{
	struct bq2419x *bq = i2c_get_clientdata(client);

	free_irq(bq->irq, bq);
	sysfs_remove_group(&bq->dev->kobj, &bq2419x_attr_group);
	//bq2419x_psy_unregister(bq);  //moxizheng
	cancel_work_sync(&bq->irq_work);
...
static struct of_device_id bq2419x_match_table[] = {
	{ .compatible = "bq24192", },
	{ },
};

static struct i2c_driver bq2419x_charger = {
	...
	.driver		= {
		.name	= "bq24192",
		.owner    = THIS_MODULE,
		.of_match_table = bq2419x_match_table,
	},
};
...
#define USB_CT_THRESHOLD_MA	1000
...
*/
    ...
    if(get_bq2419())
    {
	    return bq2419x_charge_type(get_bq2419());
    }
	return POWER_SUPPLY_CHARGE_TYPE_NONE;
    //dengyj CR01389404 end
...
*/
       ...
       bq2419_batt_health = get_prop_batt_health(chip);
       if(bq2419_batt_health == POWER_SUPPLY_HEALTH_OVERHEAT || bq2419_batt_health == POWER_SUPPLY_HEALTH_COLD)
       {
	        return POWER_SUPPLY_STATUS_DISCHARGING;
       }
       ...
       if(get_bq2419())
       {
	        return bq2419x_charge_status(get_bq2419());
       }
...
qpnp_batt_external_power_changed(struct power_supply *psy)
{
...
#ifdef GN_BQ24192
        struct bq2419x *bq = get_bq2419();//dengyj CR01388955
#endif
	...
	if (!qpnp_chg_is_usb_chg_plugged_in(chip)) {
            ...
            pr_debug("bq2419 charger out \n");
            if(bq)
            {
                bq->charger_type = BQ2419X_VBUS_NONE;
		        wake_unlock(&bq->charger_wake_lock);
            }
    }
#endif
    //dengyj CR01388955 end
	...
	if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
        ...
        if(bq)
        {
            pr_debug("bq2419 schedule charger_work \n");
	    
            if((ret.intval / 1000) > USB_CT_THRESHOLD_MA)
            {
                bq->charger_type = BQ2419X_VBUS_TA_CHARGER;

            }
        //yangshi 20141119 add for CR01414190 begin
            if((ret.intval / 1000 < USB_CT_THRESHOLD_MA) && (ret.intval / 1000 > USB_CT_THRESHOLD_MA))
            {
                bq->charger_type = BQ2419X_VBUS_CT_CHARGER;
            }
        //yangshi 20141119 add for CR01414190 end

            else if(ret.intval <= 2)
            {
                bq->charger_type = BQ2419X_VBUS_NONE;
            }
            else
            {
                bq->charger_type = BQ2419X_VBUS_USB_CHARGER;
            }
		    wake_lock(&bq->charger_wake_lock);
	        cancel_delayed_work_sync(&bq->charger_work);
            schedule_delayed_work(&bq->charger_work,
...
//Gionee wudp 2013-09-10 add for charger voltage end
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
        ...
        if(bq)
        {
		    val->intval = !(bq->bq2419_charging_disabled);
        }
        else
        {
            val->intval = 0;
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
        ...
        if(bq)
        {
		    bq->bq2419_charging_disabled = !(val->intval);
		    if (bq->bq2419_charging_disabled) {
			    /* disable charging */
                bq2419x_charger_enable(false);
		    } else {
			    /* enable charging */
...
qpnp_charger_probe(struct spmi_device *spmi)
{
    ...
    //dengyj CR01396960 begin
#ifndef GN_BQ24192
	schedule_delayed_work(&chip->aicl_check_work,
		msecs_to_jiffies(EOC_CHECK_PERIOD_MS));
#endif
    //dengyj CR01396960 end

#ifdef GN_BQ24192
    g_chip = chip;//dengyj CR01389404
#endif
...
{
//moxizhzheng add 
	i2c_add_driver(&bq2419x_charger);
