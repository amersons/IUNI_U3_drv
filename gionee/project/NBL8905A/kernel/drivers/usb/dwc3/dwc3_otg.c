		...
		if (!test_bit(ID, &dotg->inputs)) {
			...
			if (charger) {
				...
				case DWC3_DCP_CHARGER:
					...
					pm_runtime_put_sync(phy->dev);
					break;
				//gionee yangshi 20141113 add for CR01409985 begin
				case DWC3_PROPRIETARY_CHARGER:
                                        dwc3_otg_set_power(phy, 900); //yangshi 20141119 modify for CR01414190
                                        pm_runtime_put_sync(phy->dev);
                                        break;
				//gionee yangshi 20141113 add for CR01409985 end
