...
//Gionee licz add for debug
#include <linux/time.h>
#include <linux/rtc.h>

#define DT_CMD_HDR 6

//Gionee licz add for device check begin
#ifdef CONFIG_GN_Q_BSP_DEVICE_TYPE_CHECK_SUPPORT
#include <linux/gn_device_check.h>
struct gn_device_info mdss_panel_info;
extern void gn_set_device_info(struct gn_device_info gn_dev_info);
enum lcm_type{
MDSS_DSI_PANEL_NONE,
MDSS_DSI_PANEL_NT35596_TRULY,
MDSS_DSI_PANEL_R63315_SHARP,
MDSS_DSI_PANEL_NT35695_TRULY,
};

static int mdss_dsi_panel_lcm_type = MDSS_DSI_PANEL_NONE;

#endif
//Gionee licz add for device check end
//Gionee licz add for debug begin

static struct timespec ts;
static struct rtc_time tm;

//Gionee licz add for debug end
...
static char dcs_cmd[2] = {0x0a, 0x00}; /* DTYPE_DCS_READ */
//static char dcs_cmd[2] = {0xf4, 0x00}; /* DTYPE_DCS_READ */
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (!pinfo->panel_power_on) {
			...
			//caomingliang add end
			//caomingliang add for turly test 
			if (mdss_dsi_panel_lcm_type == MDSS_DSI_PANEL_NT35596_TRULY) {
							...
							u32 bl_level)
{
	...
	//Gionee licz add for debug end
        	...
        	pr_info("%s: bl_level=%d, custdown level=%d at %d-%02d-%02d %02d:%02d:%02d.%09lu UTC\n",__func__,bl_level,((bl_level*400)/525),
                        tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                        tm.tm_hour, tm.tm_min, tm.tm_sec, ts.tv_nsec);
#ifdef CONFIG_GN_Q_BSP_DEVICE_TYPE_CHECK_SUPPORT
	if (mdss_dsi_panel_lcm_type == MDSS_DSI_PANEL_NT35596_TRULY) {
		bl_level = 400 * bl_level / 480;//add for  lcd brightness
	}else if (mdss_dsi_panel_lcm_type == MDSS_DSI_PANEL_R63315_SHARP) {
		bl_level = 400 * bl_level / 560;//add for  lcd brightness
	}
#endif
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	//Gionee licz add for debug begin
        ...
        pr_info("%s: enter at %d-%02d-%02d %02d:%02d:%02d.%09lu UTC\n",__func__,
                        tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                        tm.tm_hour, tm.tm_min, tm.tm_sec, ts.tv_nsec);
	//Gionee licz add for debug end
	ctrl = container_of(pdata, struct mdss_dsi_ctrl_pdata,
	...
	//Gionee licz add for debug begin
        ...
        pr_info("%s: exit at %d-%02d-%02d %02d:%02d:%02d.%09lu UTC\n",__func__,
                        tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                        tm.tm_hour, tm.tm_min, tm.tm_sec, ts.tv_nsec);
	//Gionee licz add for debug end
	pr_debug("%s:-\n", __func__);
...
#ifdef CONFIG_GN_Q_BSP_DEVICE_TYPE_CHECK_SUPPORT
        int i;
        static char panel_ic_name[GN_DEVICE_NAME_LEN];
        static const char *p_panel_name = NULL;
#endif
        //Gionee licz add for device check end
...
//Gionee licz add for device check begin
#ifdef CONFIG_GN_Q_BSP_DEVICE_TYPE_CHECK_SUPPORT
        for(p_panel_name=panel_name, i=0; i<256; i++) {
                if(p_panel_name[i] == ' ') {
                        break;
                }
                panel_ic_name[i] = p_panel_name[i];
        }
        panel_ic_name[i]='\0';


        if(!strcmp(panel_ic_name, "nt35596")) {
                strcpy(mdss_panel_info.vendor, "Truly");
        	strcpy(mdss_panel_info.name, "truly nt35596");
		mdss_dsi_panel_lcm_type = MDSS_DSI_PANEL_NT35596_TRULY;
        } else if(!strcmp(panel_ic_name, "r63311")) {
                strcpy(mdss_panel_info.vendor, "Ofilm");
        	strcpy(mdss_panel_info.name, "sharp r63315");
		mdss_dsi_panel_lcm_type = MDSS_DSI_PANEL_R63315_SHARP;
	} else if(!strcmp(panel_ic_name, "nt35695")) {
                strcpy(mdss_panel_info.vendor, "Truly");
        	strcpy(mdss_panel_info.name, "truly nt35695");
		mdss_dsi_panel_lcm_type = MDSS_DSI_PANEL_NT35695_TRULY;
        } else {
                strcpy(mdss_panel_info.vendor, "Unknow");
        }
        strcpy(mdss_panel_info.version, "V1.0");

        mdss_panel_info.gn_dev_type = GN_DEVICE_TYPE_LCD;

        pr_info("%s: panel_ic_name %s, vendor %s, gn_dev_type %d\n",__func__,
                                panel_ic_name,
                                mdss_panel_info.vendor,
                                mdss_panel_info.gn_dev_type);

        gn_set_device_info(mdss_panel_info);
