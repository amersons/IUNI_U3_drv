...
/* Gionee Driver, XiongLin, 20140810, add for ES9018 HiFi chip. --> */
static int msm8974_sec_mi2s_free_gpios(void)
{
	int i;
...
static void msm8974_mi2s_shutdown(struct snd_pcm_substream *substream)
{
	...
	#else
		if (sec_mi2s_clk.bit_clk) {
			clk_disable_unprepare(sec_mi2s_clk.bit_clk);
			clk_put(sec_mi2s_clk.bit_clk);
			sec_mi2s_clk.bit_clk = NULL;
		}

		if (sec_mi2s_clk.osr_clk) {
			clk_disable_unprepare(sec_mi2s_clk.osr_clk);
			clk_put(sec_mi2s_clk.osr_clk);
			sec_mi2s_clk.osr_clk = NULL;
		}
		if (sec_mi2s_clk.core_clk) {
			clk_put(sec_mi2s_clk.core_clk);
			sec_mi2s_clk.core_clk = NULL;
		}
	#endif

		msm8974_sec_mi2s_free_gpios();
	}
}
...
static int msm8974_mi2s_startup(struct snd_pcm_substream *substream)
{
	...
	if (atomic_inc_return(&sec_mi2s_clk.mi2s_rsc_ref) == 1) {
		...
		msm8974_configure_sec_mi2s_gpio();
	#ifdef MI2S_LPASS_CLK_ENABLE
	...
	#else
		/* Set up core clock. */
		pr_info(" %s Setting up the clocks ... \n", __func__);
		sec_mi2s_clk.core_clk = clk_get(cpu_dai->dev, "core_clk");
		if (IS_ERR(sec_mi2s_clk.core_clk)) {
			dev_err(cpu_dai->dev, "%s: Failed to Core clk %ld\n"
					"CPU dai name %s\n", __func__,
					PTR_ERR(sec_mi2s_clk.core_clk),
					cpu_dai->dev->driver->name);
			return -ENODEV ;
		}
		/* osr clock */
		sec_mi2s_clk.osr_clk = clk_get(cpu_dai->dev, "osr_clk");
		if (IS_ERR(sec_mi2s_clk.osr_clk)) {
			dev_err(cpu_dai->dev, "%s: Failed to request OSR %ld\n"
					"CPU dai name %s\n", __func__,
					PTR_ERR(sec_mi2s_clk.osr_clk),
					cpu_dai->dev->driver->name);
			clk_put(sec_mi2s_clk.core_clk);
			return -ENODEV ;
		}
		clk_set_rate(sec_mi2s_clk.core_clk, 12288000);
		ret = clk_prepare_enable(sec_mi2s_clk.osr_clk);
		if (ret != 0) {
			dev_err(cpu_dai->dev, "Fail to enable cdc_osr_clk\n");
			goto exit_osrclk_err;
		}
		/* ibit clock */
		sec_mi2s_clk.bit_clk = clk_get(cpu_dai->dev, "ibit_clk");
		if (IS_ERR(sec_mi2s_clk.bit_clk)) {
			dev_err(cpu_dai->dev, "%s: Failed to request Bit %ld\n"
					"CPU dai name %s\n", __func__,
					PTR_ERR(sec_mi2s_clk.bit_clk),
					cpu_dai->dev->driver->name);
			clk_put(sec_mi2s_clk.core_clk);
			clk_put(sec_mi2s_clk.osr_clk);
			return -ENODEV ;
		...
		//clk_set_rate(sec_mi2s_clk.core_clk, 12288000);
		//clk_set_rate(sec_mi2s_clk.osr_clk, 12288000);
		clk_set_rate(sec_mi2s_clk.bit_clk, 7);
		...
		ret = clk_prepare_enable(sec_mi2s_clk.bit_clk);
		if (ret != 0) {
