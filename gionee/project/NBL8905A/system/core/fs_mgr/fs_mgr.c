...
#define GN_Q_BSP_ONLINE_RESIZE_DATA_SUPPORT
#ifdef GN_Q_BSP_ONLINE_RESIZE_DATA_SUPPORT
#define RESIZE_DATA_BIN "/system/bin/ext4_resize"
#endif
...
//Gionee chuqf 2014-07-03 for online resize data image, begin
#ifdef GN_Q_BSP_ONLINE_RESIZE_DATA_SUPPORT
static int online_resize_data_img(void)
{
    int status;
    int ret = 0;
    char *e2resize_argv[] = {
    ...
    ret = android_fork_execvp_ext(ARRAY_SIZE(e2resize_argv), e2resize_argv, &status, true, LOG_KLOG | LOG_FILE, true, FSCK_LOG_FILE);
...
//Gionee chuqf 2014-07-03 for online resize data image, begin
#ifdef GN_Q_BSP_ONLINE_RESIZE_DATA_SUPPORT
        if(strcmp("/dev/block/platform/msm_sdcc.1/by-name/userdata", fstab->recs[i].blk_device) == 0){
            INFO("resize data partition\n");
            online_resize_data_img();
        }
#endif
//Gionee chuqf 2014-07-03 for online resize data image, end
