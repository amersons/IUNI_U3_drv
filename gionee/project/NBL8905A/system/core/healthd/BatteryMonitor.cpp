...
bool BatteryMonitor::update(void) {
    ...
    props.usbHealth = BATTERY_HEALTH_UNKNOWN;//dengyj CR01272874
...
//dengyj begin CR01272874
    if (readFromFile(mHealthdConfig->usbHealthPath, buf, SIZE) > 0)
        props.usbHealth = getBatteryHealth(buf);
    //dengyj end CR01272874
    ...
    if(props.chargerAcOnline || props.chargerUsbOnline)
    {
        if((props.batteryStatus != BATTERY_STATUS_CHARGING) && (props.batteryStatus != BATTERY_STATUS_FULL)
           && (props.batteryHealth != BATTERY_HEALTH_COLD) && (props.batteryHealth != BATTERY_HEALTH_OVERHEAT))
        {
            if(logthis)
            {
            KLOG_INFO(LOG_TAG,"set online to false by force!\n");
            }
            props.chargerAcOnline = false;
            props.chargerUsbOnline = false;
    ...
    //dengyj CR01374990 end

    if (logthis) {
        char dmesgline[256];
        snprintf(dmesgline, sizeof(dmesgline),
                 "battery l=%d v=%d t=%s%d.%d h=%d usbhealth=%d,st=%d",//dengyj CR01272874
                 props.batteryLevel, props.batteryVoltage,
                 props.batteryTemperature < 0 ? "-" : "",
                 abs(props.batteryTemperature / 10),
                 abs(props.batteryTemperature % 10), props.batteryHealth,props.usbHealth,//dengyj CR01272874
                 props.batteryStatus);
    ...
    if (dir == NULL) {
        ...
        while ((entry = readdir(dir))) {
            ...
            if(strcmp(name,"usb")==0)
            {
                	...
                	if (access(path.string(), R_OK) == 0)
                        mHealthdConfig->usbHealthPath = path;
...
//dengyj begin CR01272874
    if (mHealthdConfig->usbHealthPath.isEmpty())
    {
        KLOG_INFO(LOG_TAG, "usbHealthPath not found\n");
    }
    else
    {
        KLOG_INFO(LOG_TAG, "usbHealthPath have found is:%s\n",mHealthdConfig->usbHealthPath.string());
    }
