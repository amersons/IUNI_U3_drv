...
//Gionee <xuwen><2013-08-14> add for CR00856964 begin
static const int NAP_TIME_STEP = 20;
//Gionee <xuwen><2013-08-14> add for CR00856964 end
...
//Gionee <xuwen><2013-08-14> modify for CR00856964 begin
/*
    while (maxnaps-- > 0) {
...
*/
    int iTimes = NAP_TIME / NAP_TIME_STEP;
    int iLoop;
    
    while (maxnaps-- > 0) {
        for (iLoop = 0; iLoop < iTimes; iLoop++) {
            usleep(NAP_TIME_STEP * 1000);
            if (property_get(name, value, NULL)) {
                if (desired_value == NULL || 
                    strcmp(value, desired_value) == 0) {
                    return 0;
...
//Gionee <xuwen><2013-08-14> modify for CR00856964 end
