...
//gionee, chuqf, for usbotg, begin
#ifdef GN_VOLD_USBOTG_SUPPORT
    if (!strcmp(rec->label, "usbotg")){
...
//xiaoqian, 20141021, add for supporting the cards which has no partitions, begin	
#ifdef CONFIG_GN_Q_BSP_SUPPORT
    if(0 == mDiskNumParts) {
      handlePartitionRemoved(devpath, evt);
    }
#endif /* CONFIG_GN_Q_BSP_SUPPORT */
//xiaoqian, 20141021, add for supporting the cards which has no partitions, end
