  ...
  isp_pix_trigger_update_input_t *trigger_params)
{
  ...
  if (new_flash_mode != CAM_FLASH_MODE_ON) {
	...
	/* adjust rolloff table according to brightness, tanrifei, 20140219 */
	{	
		static tintless_mesh_rolloff_array_t cur_mesh_hw;
		static int inited = 0;
		...
		int i, j, row, col;
		float aec_ratio = 0.0;
		float lowlight_comp_ratio = 1.0;
		float luma_bias_ratio = 1.0;
		float rolloff_proof_ratio = 10.0;
		float cutoff_ratio = 1.0;
		float diff;
		...
		aec_ratio = isp_util_get_aec_ratio(mod->notify_ops->parent,
					  ...
					  &(chromatix_rolloff->rolloff_lowlight_trigger),
					  &trigger_params->trigger_input.stats_update.aec_update, is_burst);
		lowlight_comp_ratio = 0.5 + aec_ratio*0.5;

		if (trigger_params->trigger_input.stats_update.aec_update.lux_idx > 
			(trigger_params->trigger_input.stats_update.aec_update.indoor_index + 180)) {
			lowlight_comp_ratio = 0.1;
		} else if (trigger_params->trigger_input.stats_update.aec_update.lux_idx > 
			(trigger_params->trigger_input.stats_update.aec_update.indoor_index + 130)) {
			lowlight_comp_ratio = 0.1 + 0.4*((trigger_params->trigger_input.stats_update.aec_update.indoor_index + 180)
				- trigger_params->trigger_input.stats_update.aec_update.lux_idx) / 50;
		}
		
		if (trigger_params->trigger_input.stats_update.aec_update.SY != NULL) {
			int coner_index[4] = {0, 15, 240, 255};
			int center_index[4] = {119, 120, 135, 136};
			uint32_t coner_luma=255, center_luma=0;

			for (i=0; i<4; i++) {
				if (trigger_params->trigger_input.stats_update.aec_update.SY[coner_index[i]] 
					< coner_luma) {
					coner_luma = trigger_params->trigger_input.stats_update.aec_update.SY[coner_index[i]];
				}

				center_luma += trigger_params->trigger_input.stats_update.aec_update.SY[center_index[i]];
			}
			
			center_luma /= 4;
			if (center_luma > 0) {
				luma_bias_ratio = (float)coner_luma / center_luma;
				if (luma_bias_ratio > 0.5) {
					luma_bias_ratio = 1.0;
				} else {
					luma_bias_ratio = 0.5 + luma_bias_ratio*0.5/0.5;
				}
			}
		}
		...
		{			
			...
			if (chromatix_rolloff->chromatix_mesh_rolloff_table[ROLLOFF_D65_LIGHT].mesh_rolloff_table_size == 130) {
				coner_index1[0] = 0;
				coner_index1[1] = 12;
				coner_index1[2] = 117;
				coner_index1[3] = 129;
				...
				coner_index1[0] = 0;
				coner_index1[1] = 0;//16;
				coner_index1[2] = 0;//204;
				coner_index1[3] = 0;//220;
			}

			if (TINTLESS_ROLLOFF_TABLE_SIZE == 130) {
				coner_index2[0] = 0;
				coner_index2[1] = 12;
				coner_index2[2] = 117;
				coner_index2[3] = 129;
				...
				coner_index2[0] = 0;
				coner_index2[1] = 0;//16;
				coner_index2[2] = 0;//204;
				coner_index2[3] = 0;//220;
			}
			
			for (i=0; i<4; i++) {
				tmp1 += Q_TO_FLOAT(10, meshRolloffTableAEC.TableR[coner_index1[i]]); 
				tmp2 +=	mesh_table.r_gain[coner_index2[i]];
			}
			rolloff_proof_ratio = tmp1 / tmp2;
		}
		
		cutoff_ratio = (lowlight_comp_ratio<luma_bias_ratio) ? lowlight_comp_ratio: luma_bias_ratio;
		//cutoff_ratio = 0.6*lowlight_comp_ratio + 0.4*luma_bias_ratio;
		cutoff_ratio = (cutoff_ratio<rolloff_proof_ratio) ? cutoff_ratio : rolloff_proof_ratio;
		CDBG("cutoff_ratio = %f, lowlight_comp_ratio = %f, luma_bias_ratio = %f, rolloff_proof_ratio = %f", 
			cutoff_ratio, lowlight_comp_ratio, luma_bias_ratio, rolloff_proof_ratio);
		...
		for (i=0; i<((row+1)/2); i++) {
			for (j=0; j<((col+1)/2); j++) {
				mesh_table.r_gain[i*col+j] = (mesh_table.r_gain[i*col+j] + mesh_table.r_gain[(i+1)*col-j-1] +
					mesh_table.r_gain[(row-1-i)*col+j] + mesh_table.r_gain[(row-i)*col-j-1]) / 4;
				mesh_table.r_gain[i*col+j] = 1.0 + (mesh_table.r_gain[i*col+j]-1.0)*cutoff_ratio;
				mesh_table.r_gain[(i+1)*col-j-1] = mesh_table.r_gain[i*col+j];
				mesh_table.r_gain[(row-1-i)*col+j] = mesh_table.r_gain[i*col+j];
				mesh_table.r_gain[(row-i)*col-j-1] = mesh_table.r_gain[i*col+j];

				mesh_table.gr_gain[i*col+j] = (mesh_table.gr_gain[i*col+j] + mesh_table.gr_gain[(i+1)*col-j-1] +
					mesh_table.gr_gain[(row-1-i)*col+j] + mesh_table.gr_gain[(row-i)*col-j-1]) / 4;
				mesh_table.gr_gain[i*col+j] = 1.0 + (mesh_table.gr_gain[i*col+j]-1.0)*cutoff_ratio;
				mesh_table.gr_gain[(i+1)*col-j-1] = mesh_table.gr_gain[i*col+j];
				mesh_table.gr_gain[(row-1-i)*col+j] = mesh_table.gr_gain[i*col+j];
				mesh_table.gr_gain[(row-i)*col-j-1] = mesh_table.gr_gain[i*col+j];
				
				mesh_table.gb_gain[i*col+j] = (mesh_table.gb_gain[i*col+j] + mesh_table.gb_gain[(i+1)*col-j-1] +
					mesh_table.gb_gain[(row-1-i)*col+j] + mesh_table.gb_gain[(row-i)*col-j-1]) / 4;
				mesh_table.gb_gain[i*col+j] = 1.0 + (mesh_table.gb_gain[i*col+j]-1.0)*cutoff_ratio;
				mesh_table.gb_gain[(i+1)*col-j-1] = mesh_table.gb_gain[i*col+j];
				mesh_table.gb_gain[(row-1-i)*col+j] = mesh_table.gb_gain[i*col+j];
				mesh_table.gb_gain[(row-i)*col-j-1] = mesh_table.gb_gain[i*col+j];

				mesh_table.b_gain[i*col+j] = (mesh_table.b_gain[i*col+j] + mesh_table.b_gain[(i+1)*col-j-1] +
					mesh_table.b_gain[(row-1-i)*col+j] + mesh_table.b_gain[(row-i)*col-j-1]) / 4;
				mesh_table.b_gain[i*col+j] = 1.0 + (mesh_table.b_gain[i*col+j]-1.0)*cutoff_ratio;
				mesh_table.b_gain[(i+1)*col-j-1] = mesh_table.b_gain[i*col+j];
				mesh_table.b_gain[(row-1-i)*col+j] = mesh_table.b_gain[i*col+j];
				mesh_table.b_gain[(row-i)*col-j-1] = mesh_table.b_gain[i*col+j];				
			}
		}
		...
		if (!inited) {
			...
			for (i=0; i<TINTLESS_ROLLOFF_TABLE_SIZE; i++) {
				mesh_table.r_gain[i] = cur_mesh_hw.r_gain[i]*0.9 + mesh_table.r_gain[i]*0.1;
				mesh_table.gr_gain[i] = cur_mesh_hw.gr_gain[i]*0.9 + mesh_table.gr_gain[i]*0.1;
				mesh_table.gb_gain[i] = cur_mesh_hw.gb_gain[i]*0.9 + mesh_table.gb_gain[i]*0.1;
				mesh_table.b_gain[i] = cur_mesh_hw.b_gain[i]*0.9 + mesh_table.b_gain[i]*0.1;
			}
		}
		CDBG("corner r_gain : %f, %f, %f, %f", 
			mesh_table.r_gain[0], mesh_table.r_gain[12], mesh_table.r_gain[117], mesh_table.r_gain[129]);
		memcpy(&cur_mesh_hw, &mesh_table, sizeof(cur_mesh_hw));
	}
	/* add end */
