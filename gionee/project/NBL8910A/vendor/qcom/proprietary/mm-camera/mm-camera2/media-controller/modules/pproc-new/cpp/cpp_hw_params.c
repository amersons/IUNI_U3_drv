  ...
  struct cpp_frame_info_t *in_frame_info)
{
  ...
  fw_version_1_2_x = cpp_hardware_fw_version_1_2_x(cpphw);
  ...
  /*frame info size*/
  msg_len = (126 + (2 * fw_version_1_2_x))+ 27 * num_stripes + 3;
  ...
  ASF_7x7_light_type type1, ASF_7x7_light_type type2, float interpolate_factor)
{
  ...
  for (i = 0; i < CPP_PARAM_ASF_LUT_MAX; i++) {
    ...
    CDBG("%s:%d lut1[%d] %f = ((factor %f * reference_iplus1 %f) +"
    ...
    CDBG("%s:%d lut2[%d] %f = ((factor %f * reference_iplus1 %f) +"
      "((1 - factor) %f * reference_i %f))\n",
  ...
  for (i = 0; i < CPP_PARAM_ASF_LUT3_MAX; i++) {
    ...
    CDBG("%s:%d lut3[%d] %f = ((factor %f * reference_iplus1 %f) +"
      "((1 - factor) %f * reference_i %f))\n",
  ...
  cpp_params_aec_trigger_t *aec_trigger_params)
{
  ...
  if (hw_params->asf_mode == CPP_PARAM_ASF_DUAL_FILTER || hw_params->scene_mode_on) {
    ...
    asf_info->nz_flag = asf_7_7->nz[0];

    CDBG("%s:%d min_ds %f max_ds %f max_factor %f\n", __func__, __LINE__,
  ...
  ReferenceNoiseProfile_type *ref_noise_profile_iplus1)
{
  ...
  for (k = 0; k < CPP_DENOISE_NUM_PROFILES; k++) {
	...
	/* impose less denoise weight if in preview mode, tanrifei, 20140814  */
	if (cpp_hw_params->stream_type == CAM_STREAM_TYPE_PREVIEW) {
		cpp_hw_params->denoise_info[0][k].weight += 0.20;
	} 
	/* end */
  ...
  cpp_hardware_params_t *hw_params)
{
  ...
  if (!hw_params->crop_info.process_window_width ||
    ...
    CDBG_ERROR("%s:%d failed: process window %d %d output %d %d\n",
      __func__, __LINE__, hw_params->crop_info.process_window_width,
  ...
  struct cpp_frame_info_t *frame_info)
{
 ...
 /* Hack to equalize the snpashots in ZSL and NON-ZSl mode when flip is not 0 */
  ...
  CDBG("%s:%d, punit: input_fmt=%d output_fmt=%d ", __func__, __LINE__,
  ...
  asfsharpness7x7_t *diag_cpyto_params)
{
  ...
  diag_cpyto_params->smoothpercent = diag_cpyfrm_params->sp;
  ...
  diag_cpyto_params->clamp_scale_max = diag_cpyfrm_params->clamp_scale_max;
  diag_cpyto_params->clamp_scale_min = diag_cpyfrm_params->clamp_scale_min;
  ...
  for (i = 0; i < CPP_PARAM_ASF_F_KERNEL_MAX; i++) {
    /* Update F1 kernel */
    diag_cpyto_params->sobel_h_coeff[i] = diag_cpyfrm_params->sobel_h_coeff[i];
    /* Update F2 kernel */
    diag_cpyto_params->sobel_v_coeff[i] = diag_cpyfrm_params->sobel_v_coeff[i];
    /* Update F3 kernel */
    diag_cpyto_params->hpf_h_coeff[i] = diag_cpyfrm_params->hpf_h_coeff[i];
    /* Update F4 kernel */
    diag_cpyto_params->hpf_v_coeff[i] = diag_cpyfrm_params->hpf_v_coeff[i];
    /* Update F5 kernel */
    diag_cpyto_params->lpf_coeff[i] = diag_cpyfrm_params->lpf_coeff[i];
  }

  for (i = 0; i < CPP_PARAM_ASF_LUT_MAX; i++) {
    diag_cpyto_params->lut1[i] = diag_cpyfrm_params->lut1[i];
    diag_cpyto_params->lut2[i] = diag_cpyfrm_params->lut2[i];
  }
  for (i = 0; i < CPP_PARAM_ASF_LUT3_MAX; i++) {
    diag_cpyto_params->lut3[i] = diag_cpyfrm_params->lut3[i];
  }
  ...
  wavelet_t *diag_cpyto_params)
{
  ...
  for (i = 0; i < CPP_DENOISE_NUM_PROFILES; i++) {
    ...
    diag_cpyto_params->bilateral_scalecore0[i] =
      frame_info->bf_info[0].bilateral_scale[i];
    diag_cpyto_params->weightcore0[i] = frame_info->bf_info[0].weight[i];
    diag_cpyto_params->noise_thresholdcore0[i] =
      frame_info->bf_info[0].noise_threshold[i];

    /* Plane 2 */
    diag_cpyto_params->bilateral_scalecore1[i] =
      frame_info->bf_info[1].bilateral_scale[i];
    diag_cpyto_params->weightcore1[i] = frame_info->bf_info[1].weight[i];
    diag_cpyto_params->noise_thresholdcore1[i] =
      frame_info->bf_info[1].noise_threshold[i];

    /* Plane 3 */
    diag_cpyto_params->bilateral_scalecore2[i] =
      frame_info->bf_info[2].bilateral_scale[i];
    diag_cpyto_params->weightcore2[i] = frame_info->bf_info[2].weight[i];
    diag_cpyto_params->noise_thresholdcore2[i] =
      frame_info->bf_info[2].noise_threshold[i];
  }
  return;
}
