...
static int af_actuator_set_default_focus(void *ptr)
{
  ...
  /* added by tanrifei, 20140829 */
  af_actuator_ptr->curr_3a_step_pos = 0;
  /* add end */

  /* add by tanrifei, 20140822 */
  {
	  int default_pos;

	  default_pos = af_tune_ptr->af_algo.position_far_end - 
	  	af_tune_ptr->af_algo.position_normal_hyperfocal;

	  if (default_pos > 0) {
		  cfg.cfgtype = CFG_MOVE_FOCUS;
		  cfg.cfg.move.dir = MOVE_NEAR;
		  cfg.cfg.move.sign_dir = 1;
		  cfg.cfg.move.num_steps = default_pos;
		  cfg.cfg.move.dest_step_pos = default_pos;
		  curr_scene = 0;
		  /* Determine scenario */
		  scenario_size = af_tune_ptr->actuator_tuned_params.
		  ...
		  for (index = 0; index < scenario_size; index++) {
			if (af_actuator_ptr->curr_step_pos <=
			  af_tune_ptr->actuator_tuned_params.
			  ringing_scenario[MOVE_NEAR][index]) {
			  curr_scene = index;
			  break;
			}
		  }
		  cfg.cfg.move.ringing_params =
			&(af_tune_ptr->actuator_tuned_params.
			damping[MOVE_NEAR][curr_scene].ringing_params[0]);
		  ...
		  af_actuator_ptr->curr_step_pos = default_pos;		  
		  af_actuator_ptr->curr_3a_step_pos = default_pos;
	  }
  }
  /* add end */
...
static int af_actuator_move_focus(void *ptr, void *data)
{
  ...
  /* modified by tanrifei, 20140829 */
  #if 0
  dest_step_pos = af_actuator_ptr->curr_step_pos +
    (sign_dir * num_steps);
  #else 
  dest_step_pos = af_actuator_ptr->curr_3a_step_pos +
    (sign_dir * num_steps);
  #endif
  /* modify end */
  ...
  /* add by tanrifei, 20140829 */
  {
  	...
  	hyper_pos = af_tune_ptr->af_algo.position_far_end - 
		af_tune_ptr->af_algo.position_normal_hyperfocal;

	dest_3a_step_pos = dest_step_pos;
	if (dest_step_pos < hyper_pos) {
	  dest_step_pos = hyper_pos;
	}	  
  }
  /* add end */
  ...
  /* add by tanrifei, 20140829 */
  af_actuator_ptr->curr_3a_step_pos = dest_3a_step_pos;
  /* add end */
