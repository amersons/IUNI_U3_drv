...
const struct cct_trig_t back_camera_bright_cct_trig[] = {
	//{3200, {{450, 180, -1, -1},{180, 140, 1.9000, 1.6000},{140, 0, 2.0000, 1.5300}}},
	//{3800, {{450, 200, -1, -1},{200, 140, 1.9000, 1.6000},{140, 0, 2.0000, 1.5300}}},
	//{4250, {{450, 260, -1, -1},{260, 140, 1.9000, 1.6000},{140, 0, 1.9500, 1.5600}}},
	//{4400, {{450, 260, -1, -1},{260, 140, 1.9000, 1.6000},{140, 0, 2.0000, 1.5300}}}, 
	{3200, {{450, 260, -1, -1},{260, 140,	  -1,	  -1},{140, 80,		-1, 	-1},{80,  0,	 -1,	 -1}}},
	{4800, {{450, 260, -1, -1},{260, 140,	  -1,	  -1},{140, 80,		-1, 	-1},{80,  0,	 -1,	 -1}}},
};			

const struct cct_trig_t front_camera_bright_cct_trig[] = {
	{3200, {{500, 400, -1, -1},{400, 300,	  -1,	  -1},{300, 180,	  -1,	 -1},{180, 140, 2.4500, 1.6200},{140, 0, 2.4000, 1.6200}}},
	{3800, {{500, 400, -1, -1},{400, 300,	  -1,	  -1},{300, 200,	  -1,	 -1},{200, 140, 2.4500, 1.6200},{140, 0, 2.4000, 1.6200}}},
	{4100, {{500, 400, -1, -1},{400, 300,	  -1,	  -1},{300, 240,	  -1,	 -1},{240, 200, 2.4500, 1.6200},{200, 0, 2.4000, 1.6200}}}, 			
	{4600, {{500, 400, -1, -1},{400, 300,	  -1,	  -1},{300, 260,	  -1,	 -1},{260, 200, 2.4500, 1.6800},{200, 0, 2.4000, 1.6800}}},
	{4800, {{500, 400, -1, -1},{400, 300,	  -1,	  -1},{300, 280,	  -1,	 -1},{280, 200, 2.4500, 1.6800},{200, 0, 2.4000, 1.6800}}},
	{5000, {{500, 400, -1, -1},{400, 300,	  -1,	  -1},{300, 240, 2.4500, 1.6800},{240, 160, 2.4500, 1.6800},{160, 0, 2.4000, 1.6800}}}, 
	{5500, {{500, 400, -1, -1},{400, 300,	  -1,	  -1},{300, 240, 2.4500, 1.6800},{240, 160, 2.4500, 1.6800},{160, 0, 2.4000, 1.6800}}},
};		

const struct awb_refine_para front_camera_refine_para = {
	.max_reasonable_rg = 0.75,
	.min_reasonable_bg = 0.1,
	.f_a_ref_ratio = 0.5,
	.f_a_upper_bg_coe = 0.03,
	.cwf_cf_ref_ratio = 0.5,
	.cf_upper_ref_ratio = 0.2,
	.cf_left_margin = 0.98,
	.cwf_red_compensation_coe = 0.2,//0.7,
	.cwf_red_com_limit = 0.03,//0.2,
	.cwf_blue_compensation_coe = 0.0,
	.cwf_blue_com_limit = 0.0,
	.cwf_left_margin = 1.0,
	.tl84_red_compensation_coe = 0.2,
	.tl84_red_com_limit = 0.03,
	.tl84_blue_compensation_coe = 0.0,
	.tl84_blue_com_limit = 0.0,
	.a_horizon_ref_ratio_rg = 0.6,//0.5,
	.a_horizon_ref_ratio_bg = 0.8,
	.a_red_compensation_coe = 1.0,
	.a_red_com_limit = 0.03,
	.a_blue_compensation_coe = 0.0,
	.a_blue_com_limit = 0.0,
	.d50_d65_left_ref_ratio = 0.75,
	.d50_d65_left_ref_coe = 0.8,
	.d50_d65_left_min_ratio = 0.0,
	.d50_d65_left_min_max_ratio = 0.8,
	.d50_d65_right_ref_ratio = 0.67,
	.d50_d65_right_ref_coe = 0.8,
	...
	awb_port_private_t *private, awb_green_zone_output_t *output)
{
	...
	ref_rg = chromatix->AWB_bayer_algo_data.reference[AGW_AWB_COLD_FLO].RG_ratio * ratio;
	ref_bg = chromatix->AWB_bayer_algo_data.reference[AGW_AWB_COLD_FLO].BG_ratio * ratio;
...
static void awb_port_refine_gains(void *p, awb_gain_t *rgb_gain)
{
	...
	if (rgb_gain->b_gain>0 && rgb_gain->g_gain>0 && rgb_gain->r_gain>0 && (private->new_stream_cnt>2)) {		
		...
		if (rg_ratio < chromatix->AWB_bayer_algo_data.reference[AGW_AWB_A].RG_ratio) {
			...
			if (rg_ratio > chromatix->AWB_bayer_algo_data.reference[AGW_AWB_WARM_FLO].RG_ratio) {				
				...
				if ((rg_ratio > key_point_rg_ratio) && (bg_ratio < key_point_bg_ratio)) {
					...
					ref_rg_ratio = chromatix->AWB_bayer_algo_data.reference[AGW_AWB_WARM_FLO].RG_ratio;
					ref_bg_ratio = chromatix->AWB_bayer_algo_data.reference[AGW_AWB_WARM_FLO].BG_ratio;
		...
		if (is_back_camera && (private->instant_color_temp>=4000)/* && 
		private->aec_parms.lux_idx < ((private->aec_parms.indoor_index +
				private->aec_parms.outdoor_index)/2)*/) {
			...
			if (upper_ref_bg_ratio > lower_ref_bg_ratio) {
				...
				if (rg_ratio < (ref_rg_ratio*0.99)) { 
					...
					if (blue_sky_severity > 0) {
						...
						ratio = (blue_sky_severity>0.7) ? 0.7 : blue_sky_severity;
						ref_rg_ratio = ref_rg_ratio*(1.0-blue_sky_severity) + 
							chromatix->AWB_bayer_algo_data.reference[AGW_AWB_NOON].RG_ratio*blue_sky_severity;
						ref_bg_ratio = ref_bg_ratio*(1.0-blue_sky_severity) + 
							chromatix->AWB_bayer_algo_data.reference[AGW_AWB_NOON].BG_ratio*blue_sky_severity;
						...
						//blue sky  
						ratio1 = ratio1 + (1.0-ratio1)*blue_sky_severity;
						ratio2 = ratio2 + (1.0-ratio2)*blue_sky_severity;					
						CDBG("%s, %d, ratio1 %f, ratio2 %f", __func__, __LINE__, ratio1, ratio2);
					}
					...
					if (bg_ratio > chromatix->AWB_bayer_algo_data.reference[AGW_AWB_D50].BG_ratio) {
						...
						ref_rg_ratio = ref_rg_ratio*(1.0 + 0.02*ratio);
						ref_bg_ratio = ref_bg_ratio*(1.0 - 0.02*ratio); 	
						
						if (1/*bg_ratio < chromatix->AWB_bayer_algo_data.reference[AGW_AWB_D65].BG_ratio*/) {
							if (private->aec_parms.lux_idx > (private->aec_parms.indoor_index+50)) {
								...
								ratio1 = ((private->aec_parms.indoor_index+50) - private->aec_parms.lux_idx) / 
							...
							ratio2 *= para->d50_d65_right_ref_coe;
						}
					} else {
				...
				#if 0
				if ((orig_rg_ratio<ref_rg_ratio) && (orig_bg_ratio>ref_bg_ratio) && (private->instant_color_temp>4800)) {
					float min_ratio = para->d50_d65_left_min_ratio;
					float max_ratio = 1.0;
					float weight_brightness, weight_locus;
					
					float ref_dis, dis;
					float temp1, temp2;
					
					CDBG("%s, %d, high temprature, path 4", __func__, __LINE__);

					//weight from brightness
					if (private->aec_parms.lux_idx > private->aec_parms.outdoor_index) {
						weight_brightness = 1.0;
					} else if (private->aec_parms.lux_idx > (private->aec_parms.outdoor_index-0)) {
						weight_brightness = (private->aec_parms.lux_idx - private->aec_parms.outdoor_index) / 
							(private->aec_parms.indoor_index - private->aec_parms.outdoor_index);
					} else {
						weight_brightness = 0.0;
					}
					
					//weight from location
					ref_dis = (chromatix->AWB_bayer_algo_data.reference[AGW_AWB_D75].BG_ratio - 
						chromatix->AWB_bayer_algo_data.reference[AGW_AWB_NOON].BG_ratio) *
						(chromatix->AWB_bayer_algo_data.reference[AGW_AWB_NOON].RG_ratio - 
						chromatix->AWB_bayer_algo_data.reference[AGW_AWB_D75].RG_ratio);
					dis = (ref_rg_ratio - orig_rg_ratio) * (orig_bg_ratio - ref_bg_ratio);
					
					CDBG("%s, %d, ratio %f %f, dis %f %f", __func__, __LINE__, 
						orig_rg_ratio, orig_bg_ratio, dis, ref_dis);
					...
					if (ref_dis > 0) {
						...
						if (ratio > 0.25) {
							...
							weight_locus = ratio/0.25;
						}
					}

					ratio = weight_brightness*weight_locus;
					max_ratio = para->d50_d65_left_min_max_ratio*ratio + 1.0*(1.0-ratio);

					//wieght from blue sky
					if (brightness_weighted_blue_sky_severity > 0.1) {
						ratio = 1.0;
					} else {
						ratio = brightness_weighted_blue_sky_severity*10;
					}
					...
					CDBG("%s, %d, high temprature proof ratio %f, min_ratio %f, max_ratio %f", 
						__func__, __LINE__, ratio, min_ratio, max_ratio);
				}
				#endif
		...
		#if 1
		//extreme dark situation, and bright situation, we should refine AWB gains
		if (private->instant_color_temp>0 && private->instant_color_temp<=7500 && private->aec_parms.exp_index>0) {			
			const struct cct_trig_t *bright_cct_trig;
			int bright_trig_idx_start=0, bright_trig_idx_end=0;
			int table_idx_start, table_idx_end;
			int trig_idx1, trig_idx2;
			int size;
			//awb_gain_t lowlight_def_awb, bright_def_awb;
			float left_ref_r_gain, left_ref_b_gain;
			float left_temp_r_gain1, left_temp_b_gain1, left_temp_r_gain2, left_temp_b_gain2;
			float right_ref_r_gain, right_ref_b_gain;
			float right_temp_r_gain1, right_temp_b_gain1, right_temp_r_gain2, right_temp_b_gain2;
			static float prev_lux_idx = -1;

			CDBG("%s, exp_index %d, lux_idx %f, aec_settled %d", __func__,
				private->aec_parms.exp_index, private->aec_parms.lux_idx, 
				private->aec_parms.aec_settled);

			// bright situation
			if (is_back_camera) {
				bright_cct_trig = back_camera_bright_cct_trig;
				size = sizeof(back_camera_bright_cct_trig) / sizeof(back_camera_bright_cct_trig[0]);
			} else {
				bright_cct_trig = front_camera_bright_cct_trig;
				size = sizeof(front_camera_bright_cct_trig) / sizeof(front_camera_bright_cct_trig[0]);
			}
			
			for (i=0; i<size; i++) {
				if (private->instant_color_temp <= bright_cct_trig[i].cct) {
					break;
				}
			}
			if (i == 0) {
				table_idx_start = table_idx_end = 0;
			} else if (i == size) {
				table_idx_start = table_idx_end = size-1;
			} else {
				table_idx_start = i-1;
				table_idx_end = i;
			}

			if (bright_cct_trig[table_idx_end].cct != bright_cct_trig[table_idx_start].cct) {
				ratio = (float)(bright_cct_trig[table_idx_end].cct - private->instant_color_temp) / 
					(bright_cct_trig[table_idx_end].cct - bright_cct_trig[table_idx_start].cct);				
			} else {
				ratio = 1.0;
			}

			trig_idx1 = trig_idx2 = 0;
			for (j=0; j<BRIGHT_TRIG_ENTRY; j++) {
				if (bright_cct_trig[table_idx_start].trig[j].r_gain != 0) { 				
					trig_idx1 = j;
					if ((private->aec_parms.lux_idx >= bright_cct_trig[table_idx_start].trig[j].trig_idx_end) &&
						(private->aec_parms.lux_idx <= bright_cct_trig[table_idx_start].trig[j].trig_idx_start)){
						break;
					}
				}
			}
			for (j=0; j<BRIGHT_TRIG_ENTRY; j++) {
				if (bright_cct_trig[table_idx_end].trig[j].r_gain != 0) {					
					trig_idx2 = j;
					if ((private->aec_parms.lux_idx >= bright_cct_trig[table_idx_end].trig[j].trig_idx_end) &&
						(private->aec_parms.lux_idx <= bright_cct_trig[table_idx_end].trig[j].trig_idx_start)){
						break;
					}
				}
			}

			//left reference AWB gain
			if (trig_idx1 > 0) {
				left_temp_r_gain1 = bright_cct_trig[table_idx_start].trig[trig_idx1-1].r_gain;
				left_temp_b_gain1 = bright_cct_trig[table_idx_start].trig[trig_idx1-1].b_gain;
			} else {
				left_temp_r_gain1 = rgb_gain->r_gain;
				left_temp_b_gain1 = rgb_gain->b_gain;
			}

			if (trig_idx2 > 0) {
				left_temp_r_gain2 = bright_cct_trig[table_idx_end].trig[trig_idx2-1].r_gain;
				left_temp_b_gain2 = bright_cct_trig[table_idx_end].trig[trig_idx2-1].b_gain;
			} else {
				left_temp_r_gain2 = rgb_gain->r_gain;
				left_temp_b_gain2 = rgb_gain->b_gain;
			}
			if (left_temp_r_gain1 < 0) {
				left_temp_r_gain1 = rgb_gain->r_gain;
				left_temp_b_gain1 = rgb_gain->b_gain;
			}
			if (left_temp_r_gain2 < 0) {
				left_temp_r_gain2 = rgb_gain->r_gain;
				left_temp_b_gain2 = rgb_gain->b_gain;
			}
			left_ref_r_gain = left_temp_r_gain1*ratio + left_temp_r_gain2*(1-ratio);
			left_ref_b_gain = left_temp_b_gain1*ratio + left_temp_b_gain2*(1-ratio);

			
			//right reference AWB gain
			right_temp_r_gain1 = bright_cct_trig[table_idx_start].trig[trig_idx1].r_gain;
			right_temp_b_gain1 = bright_cct_trig[table_idx_start].trig[trig_idx1].b_gain;
			right_temp_r_gain2 = bright_cct_trig[table_idx_end].trig[trig_idx2].r_gain;
			right_temp_b_gain2 = bright_cct_trig[table_idx_end].trig[trig_idx2].b_gain;
			if (right_temp_r_gain1 < 0) {
				right_temp_r_gain1 = rgb_gain->r_gain;
				right_temp_b_gain1 = rgb_gain->b_gain;
			}
			if (right_temp_r_gain2 < 0) {
				right_temp_r_gain2 = rgb_gain->r_gain;
				right_temp_b_gain2 = rgb_gain->b_gain;
			}
			right_ref_r_gain = right_temp_r_gain1*ratio + right_temp_r_gain2*(1-ratio);
			right_ref_b_gain = right_temp_b_gain1*ratio + right_temp_b_gain2*(1-ratio);

			//brightness triger border
			bright_trig_idx_start = bright_cct_trig[table_idx_start].trig[trig_idx1].trig_idx_start*ratio + 
				bright_cct_trig[table_idx_end].trig[trig_idx2].trig_idx_start*(1-ratio);
			bright_trig_idx_end = bright_cct_trig[table_idx_start].trig[trig_idx1].trig_idx_end*ratio + 
				bright_cct_trig[table_idx_end].trig[trig_idx2].trig_idx_end*(1-ratio);			
			
			CDBG("%s, cct %d, bright_trig_idx_start %d, bright_trig_idx_end %d, lux_idx %f", __func__, 
				private->instant_color_temp,
				bright_trig_idx_start, bright_trig_idx_end,
				private->aec_parms.lux_idx);
			CDBG("%s, bright situation, left_ref_r_gain %f, left_ref_b_gain %f, right_ref_r_gain %f, right_ref_b_gain %f", 
				__func__, left_ref_r_gain, left_ref_b_gain, right_ref_r_gain, right_ref_b_gain);
			
			if (private->aec_parms.lux_idx <= bright_trig_idx_start) {
				if (private->aec_parms.lux_idx <= bright_trig_idx_end) {
					ratio = 1.0;
				} else {
					ratio = (float)(bright_trig_idx_start - private->aec_parms.lux_idx) / 
						(bright_trig_idx_start - bright_trig_idx_end);
				}
				//ratio *= 0.9;
				rgb_gain->r_gain = left_ref_r_gain*(1.0-ratio) + 
					right_ref_r_gain*ratio;
				rgb_gain->b_gain = left_ref_b_gain*(1.0-ratio) + 
					right_ref_b_gain*ratio;
